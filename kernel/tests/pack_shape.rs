...
/// and the SEPARATOR record need exactly 50 too (2 + 40 + 4 + 4) -- and
...
fn pack_tree_produces_identical_tree() {
    ...
    for (i, ((name, got), want)) in cases.iter().zip(want.iter()).enumerate() {
        assert_eq!(
            got, want,
            "case {i} ({name}): the packed tree is no longer byte-identical to the \
             pre-streaming one. Fields are (root, height, first_leaf, leaves, entries, \
             ...
             Snapshot taken from the Vec-accumulating pack at e4add4b."
        );
    }
}
