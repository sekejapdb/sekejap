//! Per-hop predicates and the reaching edge: `docs/GRAPH_CONTRACT.md` §4.2
...
/// without a dependency: SplitMix64, the same shape `tests/sqlslice`'s
/// fixtures use.
struct Rng(u64);
...
struct Fixture {
    ...
    alt: GraphContextId,
    born_index: e4_prototype::collections::IndexId,
    loc_index: e4_prototype::collections::IndexId,
...
fn work_of(
...
) -> (e4_prototype::collections::QueryWork, u64) {
    ...
    let mut prepared = fixture
        ...
        .unwrap();
    let mut total = e4_prototype::collections::QueryWork::default();
    let mut rows = 0u64;
    loop {
        let page = prepared
...
fn collect(prepared: &mut e4_prototype::collections::PreparedQuery<'_>) -> Vec<EntityId> {
    let mut out = Vec::new();
    loop {
        let page = prepared
            .next_page(64, QueryBudget::unlimited(), || false)
...
fn a_budget_refusal_keeps_its_resource_across_the_atomic_boundary() {
    let raised = e4_prototype::collections::Error::BudgetExceeded {
