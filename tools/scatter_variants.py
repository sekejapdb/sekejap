...
old=b['kernel/src/btree.rs'].decode();compact=a['kernel/src/btree.rs'].decode();combined=c['kernel/src/btree.rs'].decode()
...
assert check==old
p=dict(b);p['kernel/src/btree.rs']=packing.encode()
...
for name,files in [('baseline',b),('compact',a),('packing',p),('combined',c)]:
    ...
    if name=='compact':files['kernel/tests/compact_cell_bounds.rs']=c['kernel/tests/compact_cell_bounds.rs']
    # Identical test-only path permission on all variants. No engine change.
    source=files['tests/schema_recovery.rs'].decode()
    ...
    files['tests/schema_recovery.rs']=source.encode();files['VARIANT']=(name+'\n').encode()
