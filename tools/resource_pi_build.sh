...
cargo test --offline --release -p sekejap-kernel --features sqlite-balance,compact-cells --test resource_limits -- --test-threads=1 > "$root/artifacts/resource-tests.log" 2>&1
cargo test --offline --release -p sekejap-kernel --features sqlite-balance,compact-cells --lib constrained_commit -- --test-threads=1 > "$root/artifacts/commit-fault-tests.log" 2>&1
