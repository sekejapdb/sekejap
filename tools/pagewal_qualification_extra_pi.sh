...
cp "$art/short-cap.rs" "$src/bench/src/bin/pagewal_cap.rs"
cd "$src"
cargo build -p sekejap-bench --release --offline --features sqlite-balance,compact-cells --bin pagewal_cap > "$art/short-cap-build.log" 2>&1
