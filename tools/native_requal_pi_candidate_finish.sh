...
cp "$art/candidate-pack-shape.rs" kernel/tests/pack_shape.rs
sha256sum kernel/tests/pack_shape.rs > "$art/aligned-run/candidate-final-fixture-hash.txt"
...
cargo build --release --offline --features sqlite-balance,compact-cells --bin foundation_scale --bin pagewal_bench --bin pagewal_cap --bin foundation_space > "$art/aligned-run/candidate-final-build.log" 2>&1
