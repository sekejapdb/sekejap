...
for variant in baseline candidate; do
 ...
 sha256sum kernel/src/io.rs >> "$art/aligned-run/io-hashes.txt"
 ...
 cargo build --release --offline --features sqlite-balance,compact-cells --bin foundation_scale --bin pagewal_bench --bin pagewal_cap --bin foundation_space > "$art/aligned-run/$variant-build.log" 2>&1
