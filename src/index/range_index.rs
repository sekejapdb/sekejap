...
impl RangeIndex {
    ...
    fn lower_bound(data: &[Entry], min: f64) -> usize {
        data.binary_search_by(|e| e.value.partial_cmp(&min).unwrap())
            .unwrap_or_else(|x| x)
    }
...
impl PropertyIndex for RangeIndex {
    fn insert(&self, node_idx: u32, value: &Value) {
...

        {
            ...
            let pos = data
                .binary_search_by(|e| e.value.partial_cmp(&num).unwrap())
                .unwrap_or_else(|x| x);
...
mod tests {
    ...
    fn test_range_eq() {
        ...
        assert!(
            !result.is_empty(),
            "Expected at least 1 match for value 100"
        );
        ...
        assert!(
            !result2.is_empty(),
            "Expected at least 1 match for value 200"
        );
    ...
    fn test_bulk_insert() {
        ...
        let entries: Vec<(u32, f64)> = (0..1000).map(|i| (i as u32, (i * 10) as f64)).collect();
