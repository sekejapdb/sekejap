...
use crate::collections::{corrupt, ordered, IndexId, Result};
use super::*;

/// Key tag. `[0x7A][ordered index id][term UTF-8][NUL][ordered last sequence]`.
pub(crate) const SEGMENT: u8 = 0x7A;
...
pub(crate) const SEGMENT_FEATURE: u64 = 0x40;
...
pub(crate) const MAX_SEGMENT_BYTES: usize = 3600;
...
pub(crate) fn decode(value: &[u8]) -> Result<Vec<(u64, u32)>> {
    let mut out = Vec::new();
    decode_into(value, &mut out)?;
    Ok(out)
}

pub(crate) fn decode_into(value: &[u8], out: &mut Vec<(u64, u32)>) -> Result<()> {
    out.clear();
    let mut at = 0usize;
    match value.first() {
        Some(&FORMAT) => at += 1,
...
impl Packer {
    ...
    pub(crate) fn finish(&mut self) -> Result<Option<(u64, Vec<u8>)>> {
        if self.count == 0 {
            return Ok(None);
        }
        self.take().map(Some)
...
pub(crate) fn segment_prefix(id: IndexId, term: &str) -> Vec<u8> {
...
pub(crate) fn segment_key(id: IndexId, term: &str, last: u64) -> Vec<u8> {
...
#[cfg(test)]
pub(crate) fn norm_block_fixed_width_bytes(present: usize) -> usize {
...
pub(crate) fn decode_norm_block(
    value: &[u8],
...
pub(crate) fn norm_in_block(value: &[u8], sequence: u64) -> Result<Option<u32>> {
