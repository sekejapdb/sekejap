...
use crate::collections::{
    corrupt, invalid, layout_id, ordered, read_ordered, CollectionId, Database,
...
pub(crate) const GEOMETRY_FEATURE: u64 = 0x100;
pub(crate) const GEOM_ENTRY: u8 = 0x7c;
...
/// point family's grid.
pub(crate) const LEVEL_FINE: u8 = spatial::LEVEL_FINE;
pub(crate) const LEVEL_COARSE: u8 = spatial::LEVEL_COARSE;
pub(crate) const LEVEL_WORLD: u8 = spatial::LEVEL_WORLD;
pub(crate) const MAX_CELLS: u8 = spatial::MAX_CELLS as u8;
...
pub(crate) fn posting_key_at(id: IndexId, level: u8, cell: u32, sequence: u64) -> Vec<u8> {
...
pub(crate) fn selected_geometry(document: &Value, field: &str) -> Result<Option<Geom>> {
...
pub(crate) fn indexed_bbox(geom: &Geom) -> Option<(f64, f64, f64, f64)> {
...
pub(crate) fn build_geometry_entries(
    db: &Database,
    i: &IndexInfo,
    id: EntityId,
    row: &[u8],
...
/// `maintain_indexes`.
pub(crate) fn maintain_geometry(
    db: &mut Database,
    i: &mut IndexInfo,
    id: EntityId,
    old: Option<&Value>,
...
mod tests {
    use super::*;
    use crate::collections::CollectionOptions;
    use kernel::{
        io::IoMode,
