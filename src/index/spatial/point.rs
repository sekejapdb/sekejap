...
pub(crate) const SPATIAL_FEATURE: u64 = 0x08;
pub(crate) const POINT_ENTRY: u8 = 0x74;
pub(crate) const GRID_BITS: u8 = 16;
pub(crate) const CRS_WGS84: u8 = 1;
pub(crate) const METRIC_KARNEY_V1: u8 = 1;
...
impl NearestWalk {
    ...
    pub(crate) fn note(&mut self, accepted: bool) {
        self.offered += 1;
        if accepted {
            self.accepted += 1;
        }
    ...
    pub(crate) fn rewind_past(&mut self, distance_metres: f64, id: EntityId) {
...
pub(crate) fn posting_prefix(id: IndexId) -> Vec<u8> {
...
pub(crate) fn posting_key_at(id: IndexId, cell: u32, sequence: u64) -> Vec<u8> {
...
pub(crate) fn descriptor(i: &IndexInfo) -> Result<()> {
...
pub(crate) fn selected_point(document: &Value, field: &str) -> Result<Option<Point>> {
...
pub(crate) fn point_entry(i: &IndexInfo, id: EntityId, point: Point) -> PointEntry {
    PointEntry {
...
pub(crate) fn build_point_entry(
    db: &Database,
    i: &IndexInfo,
    id: EntityId,
    row: &[u8],
...
pub(crate) fn decode_posting(prefix: &[u8], key: &[u8], value: &[u8]) -> Result<(u32, u64, Point)> {
...
#[cfg(test)]
#[path = "../../faults/spatial_fault_tests.rs"]
mod fault_tests;
