...
//! collection engine. See docs/V2_COMPAT_FIXTURES.md for the method, the
...
fn cmd_make_reference(root: &Path) -> Result<()> {
    ...
    let manifest = json!({
        ...
        "method": "raw-projection: logical KV pairs from an old kernel-Store-backed \
            ...
            Database-to-PageWalStore integration writer; see docs/V2_COMPAT_FIXTURES.md. \
...
fn snapshot_manifest(
    db: &Database,
    collections: &[(&str, e4_prototype::collections::CollectionId)],
    deleted: &[(&str, &str)],
) -> Result<Value> {
...
fn cmd_verify_and_update(root: &Path, confirmed_copy: bool) -> Result<()> {
    require(
        ...
        "verify-and-update refuses to run without --confirm-copy: this command commits \
            ...
            --confirm-copy. See docs/V2_COMPAT_FIXTURES.md.",
    )?;
