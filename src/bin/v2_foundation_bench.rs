...
//! E4 typed collections (the public `e4_prototype::collections::Database`
//! API) against SQLite with typed columns + JSONB. See
//! docs/V2_BENCHMARK_PROTOCOL.md for the full protocol, conventions reused
...
use e4_prototype::collections::{Clock, CollectionId, CollectionOptions, Database};
use e4_prototype::Kind;
use kernel::{
    io::IoMode,
...
// ---- convention already used in src/bin/collections.rs.
...
fn main() -> R<()> {
    ...
    let manifest = json!({
        ...
        "row_selection": "ascending slot order; residue-class groups documented in docs/V2_BENCHMARK_PROTOCOL.md",
        "e4": e4_report,
        "sqlite": sql_report,
    });
