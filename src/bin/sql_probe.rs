...
fn main() {
    ...
    let params: Vec<e4_prototype::sql::Param> = std::env::var("SQL_PROBE_PARAMS")
        .ok()
        .map(|v| v.split(',').map(|f| e4_prototype::sql::Param::Float(f.parse().expect("float param"))).collect())
        .unwrap_or_default();
    if let Some(n) = args.next().and_then(|n| n.parse::<usize>().ok()) {
        ...
        use e4_prototype::collections::QueryBudget;
        use std::time::Instant;
        let mut runs = Vec::with_capacity(n);
        let mut preps = Vec::with_capacity(n);
        for _ in 0..n {
            let t = Instant::now();
            let prepared = e4_prototype::sql::prepare_sql(&db, &sql, &params).expect("prepare");
