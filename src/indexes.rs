...
pub(super) fn encode(i: &IndexInfo) -> Result<Vec<u8>> {
    ...
    match i.family {
        ...
        IndexFamily::ExactVector => {
            ...
            if !(1..=super::vector_indexes::MAX_DIM).contains(&dimension) || i.unique {
        ...
        IndexFamily::SpatialPoint => {
            ...
            b.push(3);
            b.extend(i.encoding_version.to_be_bytes());
            b.push(super::spatial_indexes::GRID_BITS);
            b.push(super::spatial_indexes::CRS_WGS84);
            b.push(super::spatial_indexes::METRIC_KARNEY_V1);
        ...
        IndexFamily::Text => {
            ...
            b.extend(super::text_indexes::BM25_VERSION.to_be_bytes());
        ...
        IndexFamily::QuantizedVector => {
            ...
            b.extend(i.encoding_version.to_be_bytes());
            b.extend(u32::try_from(dimension).map_err(invalid)?.to_be_bytes());
            b.push(super::quantized_vector_indexes::QUANTIZER_VERSION);
            b.push(super::quantized_vector_indexes::OPTIONS);
            b.push(st);
            b.extend(cursor.to_be_bytes());
        }
    }
...
pub(super) fn decode(b: &[u8]) -> Result<IndexInfo> {
    ...
    let (kind, unique, state_at, cursor_at, mut at) = match family {
        ...
        IndexFamily::ExactVector => {
            ...
            if !(1..=super::vector_indexes::MAX_DIM).contains(&dimension) {
        ...
        IndexFamily::SpatialPoint => {
            ...
            if b[15] != super::spatial_indexes::GRID_BITS
                || b[16] != super::spatial_indexes::CRS_WGS84
                || b[17] != super::spatial_indexes::METRIC_KARNEY_V1
                || b[18] != 0
            {
        ...
        IndexFamily::Text => {
            ...
            if u16::from_be_bytes(b[15..17].try_into().unwrap())
                ...
                || u16::from_be_bytes(b[20..22].try_into().unwrap())
                    != super::text_indexes::BM25_VERSION
                || b[22] != 0
            {
                return Err(Error::Unsupported(
        ...
        IndexFamily::QuantizedVector => {
            ...
            if b[19] != super::quantized_vector_indexes::QUANTIZER_VERSION || b[20] != 0 {
                return Err(Error::Unsupported(
...
pub(super) fn validate_catalog(s: &PageWalStore, h: Option<IndexHeader>) -> Result<()> {
    ...
    if !h.is_some_and(|header| header.features & super::vector_indexes::VECTOR_FEATURE != 0) {
        if let Some(row) = s.range(&[super::vector_indexes::VECTOR_ENTRY])?.next() {
            let (key, _) = row?;
            if key.first() == Some(&super::vector_indexes::VECTOR_ENTRY) {
                return Err(corrupt("exact vector entries without feature admission"));
            }
        }
    }
    if !h.is_some_and(|header| header.features & super::spatial_indexes::SPATIAL_FEATURE != 0) {
        if let Some(row) = s.range(&[super::spatial_indexes::POINT_ENTRY])?.next() {
            let (key, _) = row?;
            if key.first() == Some(&super::spatial_indexes::POINT_ENTRY) {
                return Err(corrupt("spatial point entries without feature admission"));
            }
        }
    }
    if !h.is_some_and(|header| header.features & super::text_indexes::TEXT_FEATURE != 0) {
        for tag in super::text_indexes::TEXT_TAGS {
    ...
    if !h.is_some_and(|header| {
        ...
        if let Some(row) = s
            .range(&[super::quantized_vector_indexes::QUANTIZED_VECTOR_ENTRY])?
            .next()
        {
            let (key, _) = row?;
            if key.first() == Some(&super::quantized_vector_indexes::QUANTIZED_VECTOR_ENTRY) {
                return Err(corrupt(
...
impl Database {
    ...
    pub(super) fn maintain_indexes(
        ...
        for mut i in self.list_indexes(id.collection)? {
            ...
            if i.family == IndexFamily::ExactVector {
                super::vector_indexes::maintain_locator(self, &i, id, new_vectors)?;
                continue;
            }
            if i.family == IndexFamily::QuantizedVector {
                super::quantized_vector_indexes::maintain_entry(self, &i, id, new_vectors)?;
                continue;
            }
            if i.family == IndexFamily::SpatialPoint {
        ...
        for row in self.store()?.range(&start)? {
            ...
            let entry = match i.family {
                ...
                IndexFamily::ExactVector => BuiltEntry::ExactVector(
                    super::vector_indexes::build_locator(self, &i, eid, &value)?,
                ),
                IndexFamily::QuantizedVector => BuiltEntry::QuantizedVector(
                    super::quantized_vector_indexes::build_entry(self, &i, eid, &value)?,
                ),
        ...
        let result = (|| {
            ...
            if i.family == IndexFamily::Text {
                let documents = std::mem::take(&mut rows)
                    ...
                    .collect();
                super::text_indexes::build_documents(self, &i, documents)?;
            }
            for (seq, entry) in rows {
                match entry {
                    ...
                    BuiltEntry::ExactVector(Some(locator)) => self
                        .writer()?
                        .put(&super::vector_indexes::locator_key(i.id, seq), &locator)?,
                    BuiltEntry::ExactVector(None) => {}
                    BuiltEntry::QuantizedVector(Some(value)) => self.writer()?.put(
                        &super::quantized_vector_indexes::entry_key(i.id, seq),
                        &value,
                    )?,
    ...
    pub fn drop_index_step(&mut self, id: IndexId, batch: usize) -> Result<bool> {
        ...
        let (keys, end) = if i.family == IndexFamily::Text {
            super::text_indexes::drop_batch(self, id, batch)?
        } else {
