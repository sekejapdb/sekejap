...
struct Metadata {
    ...
    graph_header: Option<(graph_collections::GraphHeader, Vec<u8>)>,
    graph_names: Vec<(u8, graph_collections::GraphName, Vec<u8>)>,
}
...
fn collect_metadata(source: &SourceView) -> Result<Metadata> {
    ...
    for id in ids {
        let (index, _) = replicated_raw(source, |copy| indexes::dkey(id, copy), indexes::decode)?;
    ...
    let graph_enabled = header
        .indexes
        .is_some_and(|value| value.features & graph_collections::GRAPH_FEATURE != 0);
    let mut graph_header = None;
    let mut graph_names = Vec::new();
    if graph_enabled {
        let (decoded, bytes) = replicated_raw(
            source,
            graph_collections::graph_header_key,
            graph_collections::decode_graph_header,
        )?;
        for (kind, next) in [(0, decoded.next_type), (1, decoded.next_context)] {
            ...
            for id in 1..next {
                let (name, bytes) = replicated_raw(
                    source,
                    |copy| graph_collections::name_descriptor_key(kind, copy, id),
                    graph_collections::decode_name,
                )?;
...
fn seed_metadata(destination: &mut Destination, metadata: &Metadata) -> Result<()> {
    ...
    for index in &metadata.indexes {
        ...
        destination.put(
            &indexes::ikey(indexes::REGISTRY, index.id),
            &index.collection.0.to_be_bytes(),
        )?;
        destination.put(&indexes::ckey(index.collection, index.id), &[])?;
        let name = indexes::nkey(index.collection, &index.name);
        if destination.store.get(&name)?.is_some() {
        ...
        if index.family == IndexFamily::Text {
            destination.put(&text_indexes::corpus_key(index.id), &[0; 16])?;
        }
    }
    if let Some((_, bytes)) = &metadata.graph_header {
        put_replicas(destination, graph_collections::graph_header_key, bytes)?;
        for (kind, name, bytes) in &metadata.graph_names {
            put_replicas(
                destination,
                |copy| graph_collections::name_descriptor_key(*kind, copy, name.id),
                bytes,
            )?;
            let lookup = graph_collections::name_lookup_key(*kind, &name.name);
            ...
            match kind {
                Kind::Vector(dimension) => {
                    ...
                    } = value
                    {
                        ...
                        vector_indexes::validate_vector(&bytes, *dimension)?;
                    }
                }
                Kind::Point => {
                    if let crate::dense_v3::FieldValue::Inline(value) = value {
                        ...
                        spatial_indexes::selected_point(&Value::Object(document), name)?;
                    }
                }
                _ => {}
            }
...
fn copy_vector_sidecars(
    ...
    source.visit(&[0x60], Some(&[0x61]), |key, value| {
        ...
        vector_indexes::validate_vector(value, *dimension)?;
...
fn copy_graph(
    ...
    source.visit(
        ...
        Some(&tag_end(graph_collections::PRIMARY_EDGE)),
        |key, value| {
            let edge = graph_collections::parse_edge_key(key, graph_collections::PRIMARY_EDGE)?;
            ...
            graph_collections::decode_properties(value)?;
            ...
            destination.put(key, value)?;
            destination.put(
                &graph_collections::edge_key(graph_collections::REVERSE_EDGE, edge),
                &[],
            )?;
            edges = edges
                .checked_add(1)
    ...
    source.visit(
        ...
        Some(&tag_end(graph_collections::REVERSE_EDGE)),
        |key, _| {
            let edge = match graph_collections::parse_edge_key(key, graph_collections::REVERSE_EDGE)
            {
                Ok(edge) => edge,
                Err(_) => return Ok(()), // malformed derived garbage is discarded
            };
            if source
                .get(&graph_collections::edge_key(
                    graph_collections::PRIMARY_EDGE,
                    edge,
                ))?
                .is_none()
...
fn compare_authoritative(source: &SourceView, destination: &CurrentSourceReader) -> Result<()> {
    for tag in [0x40, 0x60, graph_collections::PRIMARY_EDGE] {
