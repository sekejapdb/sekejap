...
fn frame(kind: u32, no: u32, tx: u64, pages: u32, payload: &[u8; PAGE]) -> Vec<u8> {
    ...
    b[32..].copy_from_slice(payload);let crc=crc32c::crc32c(&b);
...
impl Pager {
    ...
    fn from_files(data:Arc<dyn FileIo>, wal:Arc<dyn FileIo>) -> Result<Arc<Self>> {
        ...
        wal.set_len(state.last_commit)?;wal.sync_full()?;data.sync_dir()?;
    ...
    fn inspect(data:&dyn FileIo,wal:&dyn FileIo)->Result<State>{
        ...
        let mut at=0;let mut last=0;let mut tx=1;let mut crc=0;
        ...
        Ok(State{latest:committed.clone(),committed:Arc::new(committed),
            end:last,last_commit:last,pages,committed_pages:pages,tx,tx_crc:0,cap:u64::MAX,free_head:0})
    }
    ...
    fn publish(&self) -> Result<()> {
        ...
        let b=frame(2,u32::MAX,s.tx,s.pages,&body);self.append(&mut s,&b)?;
        self.wal.sync_full()?;
        s.committed=Arc::new(s.latest.clone());s.committed_pages=s.pages;s.last_commit=s.end;
        s.tx=s.tx.checked_add(1).ok_or(Error::TooLarge)?;s.tx_crc=0;Ok(())
    }
    ...
    fn checkpoint_with_crash(&self, fault:u8) -> Result<bool> {
        ...
        self.wal.sync_full()?;
        s.latest.clear();s.committed=Arc::new(Index::new());s.end=0;s.last_commit=0;s.tx=1;s.tx_crc=0;
        Ok(true)
    }
...
impl PageWalStore {
    ...
    fn open_with(dir:&Path,create:bool,cache:usize,opener:impl FnOnce(&Path)->Result<Arc<Pager>>)->Result<Self>{
        ...
        let lock=std::fs::OpenOptions::new().read(true).write(true).create(true).truncate(false).open(dir.join("writer.lock"))?;
        ...
        if create {
            if s.pool.page_count()!=0{return Err(bad("create found existing pages"));}
            drop(s.pool.allocate()?);drop(s.pool.allocate()?);
    ...
    fn load_header(&mut self)->Result<()> {
        ...
        if p.kind()!=PageKind::Meta || p.nentries()!=1 || !matches!(p.slot(0).len(),16|24) || &p.slot(0)[..8]!=MAGIC {return Err(bad("pilot header"));}
        self.root=u32at(p.slot(0),8);
        if self.root<2 || self.root>=self.pool.page_count(){return Err(bad("pilot root extent"));}
        let head=u32at(p.slot(0),12);if head!=0&&(head<2||head>=self.pool.page_count()){return Err(bad("free head outside file"));}
        if let Some(pager)=&self.pager{let mut s=pager.state.lock().unwrap();s.free_head=head;
            s.cap=if p.slot(0).len()==24{u64at(p.slot(0),16)}else{u64::MAX};
    ...
    pub fn commit(&mut self)->Result<()> {
        self.writable()?;
        let result=(||{
            let (head,cap)={let s=self.pager.as_ref().unwrap().state.lock().unwrap();(s.free_head,s.cap)};
            {let mut w=self.pool.get_mut(0)?;let mut p=PageMut::init(w.bytes_mut(),PageKind::Meta,0,0);
             let mut h=MAGIC.to_vec();h.extend_from_slice(&self.root.to_le_bytes());h.extend_from_slice(&head.to_le_bytes());h.extend_from_slice(&cap.to_le_bytes());p.insert_slot(0,&h)?;p.finalise(0);}
