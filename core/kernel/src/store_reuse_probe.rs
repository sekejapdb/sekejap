...
impl FileIo for AuditIo {
    ...
    fn read_at(&self, b: &mut [u8], off: u64) -> Result<()> {
        ...
        for (i, p) in b.chunks_exact(PAGE_SIZE).enumerate() {
            if let Some(want) = self
                .writes
                .lock()
                .unwrap()
                .get(&(off + i as u64 * PAGE_SIZE as u64))
            {
                assert_eq!(
                    crc32c::crc32c(p),
                    *want,
                    "disk differs from last issued write at {}",
                    off + i as u64 * PAGE_SIZE as u64
                );
            }
        }
        Ok(())
    ...
    fn write_at(&self, b: &[u8], off: u64) -> Result<()> {
        self.file.write_at(b, off)?;
        for (i, p) in b.chunks_exact(PAGE_SIZE).enumerate() {
            self.writes
                .lock()
                .unwrap()
                .insert(off + i as u64 * PAGE_SIZE as u64, crc32c::crc32c(p));
        }
        Ok(())
    }
...
fn store_churn_audits_last_issued_page_write() {
    ...
    let io = Arc::new(AuditIo {
        file: f.into(),
        writes: Mutex::new(BTreeMap::new()),
    });
    let sync = if std::env::var_os("E4_PROBE_FULL").is_some() {
        SyncMode::Full
    } else {
        SyncMode::Off
    };
    let mut s = Store::create_on(
        d.path(),
        Config {
            budget_bytes: 8 << 20,
            io: IoMode::Buffered,
    ...
    .unwrap();
    let n = 400000u64;
    let rounds = std::env::var("E4_PROBE_ROUNDS")
    ...
    for round in 0..=rounds {
        let mut ops = 0;
        for slot in 0..n {
            let value = |id: u64, version: u64| {
                let mut v = vec![b'a' + ((id + version) % 26) as u8; 256];
                v[..8].copy_from_slice(&id.to_le_bytes());
                v[8..16].copy_from_slice(&version.to_le_bytes());
                v
            };
            if round == 0 || slot % 5 == 0 {
                s.put(&slot.to_be_bytes(), &value(slot, round)).unwrap();
                ops += 1;
            } else if slot % 10 == 1 {
                let old = if round == 1 {
                    slot
                ...
                assert!(s.delete(&old.to_be_bytes()).unwrap());
                ops += 1;
                if ops % 1000 == 0 {
                    s.checkpoint().unwrap();
                }
                s.put(
                    &(n + (round - 1) * (n / 10) + slot / 10).to_be_bytes(),
                    &value(slot, round),
                )
                .unwrap();
                ops += 1;
