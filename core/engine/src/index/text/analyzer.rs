...
use super::unicode_v1;
use std::collections::BTreeMap;

pub(crate) const ANALYZER_VERSION: u16 = 1;
pub(crate) const UNICODE_VERSION: (u8, u8, u8) = unicode_v1::UNICODE_VERSION;
...
fn push_lower(ch: char, token: &mut String) {
    ...
    match unicode_v1::LOWER.binary_search_by_key(&(ch as u32), |&(cp, _)| cp) {
        Ok(at) => token.push_str(unicode_v1::LOWER[at].1),
        Err(_) => token.push(ch),
    }
}
...
mod tests {
    ...
    fn fixed_multilingual_tokens_preserve_e3_semantics() {
        ...
        assert_ne!(unicode_v1::GENERATOR_COMPILER, "unrecorded");
    }
    #[test]
    ...
    fn table_shape_and_complete_generation_oracle() {
        for pair in unicode_v1::ALNUM.windows(2) {
            assert!(pair[0].1 + 1 < pair[1].0);
        }
        for pair in unicode_v1::LOWER.windows(2) {
