...
pub fn verify_indexed_source(
    ...
    visit(
        ...
        |key, value| {
            ...
            let info = catalog::read_index(|k| run.read(k), id)?;
    ...
    visit(
        ...
        |key, value| {
            ...
            let decoded = match catalog::decode(value) {
                Ok(decoded) => decoded,
                Err(Error::Unsupported(message)) => return Err(Error::Unsupported(message)),
                Err(_) => {
                    run.issue(VerificationIssue {
            ...
            let registry = catalog::ikey(catalog::REGISTRY, id);
    ...
    for info in &indexes {
        for copy in 0..3u8 {
            ...
            match run.read(&key)? {
                ...
                Some(bytes) => match catalog::decode(&bytes) {
        ...
        let mapped = run.read(&catalog::ckey(info.collection, info.id))?;
        run.mismatch(
            IssueClass::Catalog,
            &catalog::ckey(info.collection, info.id),
            Some(info.id),
            None,
            mapped.as_deref(),
            Some(&[]),
        ...
        let named = run.read(&catalog::nkey(info.collection, &info.name))?;
        run.mismatch(
            IssueClass::Catalog,
            &catalog::nkey(info.collection, &info.name),
            Some(info.id),
            None,
            named.as_deref(),
            Some(&want),
        ...
        for (ordinal, (name, kind)) in l.fields.iter().enumerate() {
            ...
            } = decoded
            {
                ...
                match run.read(&side_key)? {
                    Some(bytes) => {
                        if crate::index::vector::exact::validate_vector(&bytes, *dimension).is_err() {
                            run.issue(VerificationIssue {
...
fn verify_index_mappings<F: FnMut(&VerificationIssue)>(
    ...
    visit(
        &source,
        &[catalog::COLLECTION_INDEX],
        Some(&tag_end(catalog::COLLECTION_INDEX)),
        |key, value| {
            run.row(true)?;
            let mut at = 1;
            let collection =
    ...
    visit(
        &source,
        &[catalog::INDEX_NAME],
        Some(&tag_end(catalog::INDEX_NAME)),
        |key, value| {
            run.row(true)?;
            let mut at = 1;
            let collection =
...
fn verify_sidecars_and_mappings<F: FnMut(&VerificationIssue)>(run: &mut Run<F>) -> Result<()> {
    ...
    visit(&source, &[0x60], Some(&[0x61]), |key, value| {
        ...
        if crate::index::vector::exact::validate_vector(value, *dimension).is_err() {
            run.issue(VerificationIssue {
                class: IssueClass::Primary,
                kind: IssueKind::Malformed,
                key: key.to_vec(),
    ...
    match i.family {
        ...
        IndexFamily::ExactVector => {
            if let crate::dense_v3::FieldValue::Vector { ordinal, dimension } =
                field(l, row, &i.field)?
            {
                if dimension != crate::index::vector::exact::dimension(i)? {
                ...
                if crate::index::vector::exact::validate_vector(&side, dimension).is_err() {
                ...
                let expected = crate::index::vector::exact::encode_locator(l.id as u32, ordinal)?;
                let key = crate::index::vector::exact::locator_key(i.id, id.sequence);
        ...
        IndexFamily::QuantizedVector => {
            ...
            if let Some(expected) = expected {
                let key = crate::index::vector::quantized::entry_key(i.id, id.sequence);
                let actual = run.read(&key)?;
                run.mismatch(
                    IssueClass::Derived,
                    &key,
        ...
        IndexFamily::SpatialPoint => {
            ...
            if let crate::dense_v3::FieldValue::Inline(v) = f {
                ...
                if let Some(p) = crate::index::spatial::point::selected_point(&Value::Object(doc), &i.field)? {
                    let e = crate::index::spatial::point::point_entry(i, id, p);
        ...
        IndexFamily::SpatialGeometry => {
            ...
            if let crate::dense_v3::FieldValue::Inline(v) = f {
                ...
                doc.insert(i.field.clone(), v);
                if let Some(g) =
                    crate::index::spatial::geometry_index::selected_geometry(&Value::Object(doc), &i.field)?
                {
                    for e in crate::index::spatial::geometry_index::geometry_entries(i, id, &g)? {
                        let a = run.read_index(i, &e.key)?;
                        run.mismatch(
                            IssueClass::Derived,
                            &e.key,
        ...
        IndexFamily::Text => {
            ...
            if let crate::dense_v3::FieldValue::Inline(Value::String(text)) = f {
                ...
                let key = crate::index::text::norm_key(i.id, id.sequence);
                let mut actual = run.read(&key)?;
...
fn quantized_expected<F: FnMut(&VerificationIssue)>(
    ...
    if crate::index::vector::exact::validate_vector(&sidecar, dimension).is_err() {
        // The authoritative row pass reports malformed vector bytes.
        return Ok(None);
    }
    let locator =
        crate::index::vector::exact::encode_locator(u32::try_from(layout.id).map_err(corrupt)?, ordinal)?;
    crate::index::vector::quantized::encode_entry(locator, &sidecar, dimension).map(Some)
}
...
fn verify_actual<F: FnMut(&VerificationIssue)>(run: &mut Run<F>, i: &IndexInfo) -> Result<()> {
    match i.family {
        IndexFamily::Scalar => {
            let p = catalog::ikey(catalog::SCALAR, i.id);
        ...
        IndexFamily::ExactVector => {
            let p = crate::index::vector::exact::locator_prefix(i.id);
            let end = prefix_end(&p);
            let source = run.reader.clone();
            visit(&source, &p, end.as_deref(), |key, value| {
                ...
                if crate::index::vector::exact::decode_locator(value).is_err() {
                    run.issue(VerificationIssue {
                        class: IssueClass::Derived,
                        kind: IssueKind::Malformed,
                        key: key.to_vec(),
                ...
                if let Some((id, l, row)) = primary_field(run, i, seq)? {
                    ...
                    let expected = match f {
                        crate::dense_v3::FieldValue::Vector { ordinal, dimension }
                            if dimension == crate::index::vector::exact::dimension(i)? =>
                        {
                            Some(crate::index::vector::exact::encode_locator(l.id as u32, ordinal)?)
                        }
        ...
        IndexFamily::QuantizedVector => {
            let p = crate::index::vector::quantized::entry_prefix(i.id);
            let end = prefix_end(&p);
            let source = run.reader.clone();
            visit(&source, &p, end.as_deref(), |key, value| {
                run.row(true)?;
                ...
                let dimension = crate::index::vector::quantized::dimension(i)?;
                if crate::index::vector::quantized::decode_entry(value, dimension).is_err() {
                    malformed(
                        run,
                        IssueClass::Derived,
                        key,
        ...
        IndexFamily::SpatialPoint => {
            let p = crate::index::spatial::point::posting_prefix(i.id);
            let end = prefix_end(&p);
            let source = run.reader.clone();
            visit_index(&source, i, &p, end.as_deref(), |key, value| {
                run.row(true)?;
                let (_, seq, _) = match crate::index::spatial::point::decode_posting(&p, key, value) {
                    Ok(decoded) => decoded,
                    Err(_) => {
                        run.issue(VerificationIssue {
                            class: IssueClass::Derived,
                ...
                if let Some((id, l, row)) = primary_field(run, i, seq)? {
                    ...
                    let expected = if let crate::dense_v3::FieldValue::Inline(v) = f {
                        ...
                        crate::index::spatial::point::selected_point(&Value::Object(d), &i.field)?
                            .map(|p| crate::index::spatial::point::point_entry(i, id, p))
                    } else {
                        None
                    };
                    let good = expected
        ...
        IndexFamily::SpatialGeometry => {
            let p = crate::index::spatial::geometry_index::posting_prefix(i.id);
            let end = prefix_end(&p);
            let source = run.reader.clone();
            visit_index(&source, i, &p, end.as_deref(), |key, value| {
                run.row(true)?;
                let (_, _, seq, _) = match crate::index::spatial::geometry_index::decode_posting(&p, key, value) {
                    Ok(decoded) => decoded,
                    Err(_) => {
                        run.issue(VerificationIssue {
                            class: IssueClass::Derived,
                ...
                if let Some((id, l, row)) = primary_field(run, i, seq)? {
                    ...
                    let expected_entries = if let crate::dense_v3::FieldValue::Inline(v) = f {
                        let mut d = serde_json::Map::new();
                        d.insert(i.field.clone(), v);
                        match crate::index::spatial::geometry_index::selected_geometry(
                            &Value::Object(d),
                            &i.field,
                        )? {
                            Some(g) => crate::index::spatial::geometry_index::geometry_entries(i, id, &g)?,
                            None => Vec::new(),
                        }
                    } else {
                        Vec::new()
...
fn segment_tf<F: FnMut(&VerificationIssue)>(
    ...
    let prefix = crate::index::text::segments::segment_prefix(i.id, term);
    ...
    visit(&source, &start, end.as_deref(), |_, value| {
        ...
        first = false;
        if let Ok(postings) = crate::index::text::segments::decode(value) {
            found = postings
                .into_iter()
...
fn verify_text_norm_blocks<F: FnMut(&VerificationIssue)>(
    ...
    let p = crate::index::text::index_prefix(crate::index::text::segments::NORM_BLOCK, i.id);
    let end = prefix_end(&p);
    let source = run.reader.clone();
    ...
    visit(&source, &p, end.as_deref(), |key, value| {
        ...
        previous = Some(block);
        if crate::index::text::segments::decode_norm_block(value, &mut decoded).is_err() {
            return malformed(
                run,
                IssueClass::Derived,
                key,
        ...
        let base = block
            .checked_mul(crate::index::text::segments::NORM_BLOCK_SPAN)
    ...
    for (sequence, length, sampled) in entries {
        let head = run.read(&crate::index::text::norm_key(i.id, sequence))?;
        if head.is_some() {
        ...
        if let Some((id, layout, row)) = primary_field(run, i, sequence)? {
            ...
            run.mismatch(
                IssueClass::Derived,
                &crate::index::text::segments::norm_block_key(
                    i.id,
                    crate::index::text::segments::norm_block_of(sequence),
                ),
                Some(i.id),
                Some(id),
...
fn packed_norm<F: FnMut(&VerificationIssue)>(
    ...
    let key = crate::index::text::segments::norm_block_key(
        i.id,
        crate::index::text::segments::norm_block_of(sequence),
    );
    ...
    Ok(crate::index::text::segments::norm_in_block(&value, sequence).unwrap_or(None))
}
...
fn index_has_norm_blocks<F: FnMut(&VerificationIssue)>(
    ...
    let prefix = crate::index::text::index_prefix(crate::index::text::segments::NORM_BLOCK, i.id);
    let end = prefix_end(&prefix);
    let source = run.reader.clone();
    let mut any = false;
...
fn index_has_segments<F: FnMut(&VerificationIssue)>(
    ...
    let prefix = crate::index::text::index_prefix(crate::index::text::segments::SEGMENT, i.id);
    let end = prefix_end(&prefix);
    let source = run.reader.clone();
    let mut any = false;
    visit(&source, &prefix, end.as_deref(), |_, _| {
...
fn merged_df<F: FnMut(&VerificationIssue)>(
    ...
    let mut count = 0u64;
    let head = crate::index::text::posting_prefix(i.id, term);
    let head_end = prefix_end(&head);
    let source = run.reader.clone();
    visit(&source, &head, head_end.as_deref(), |_, value| {
        if crate::index::text::decode_u32(value, "text posting").is_ok_and(|tf| tf != 0) {
            count = count
                .checked_add(1)
    ...
    let segments = crate::index::text::segments::segment_prefix(i.id, term);
    let segments_end = prefix_end(&segments);
    let source = run.reader.clone();
    let mut decoded = Vec::new();
    let mut sequences = Vec::new();
    visit(&source, &segments, segments_end.as_deref(), |_, value| {
        if crate::index::text::segments::decode_into(value, &mut decoded).is_ok() {
    ...
    for sequence in sequences {
        if run
            .read(&crate::index::text::posting_key(i.id, term, sequence))?
            .is_none()
        {
            count = count
                .checked_add(1)
...
fn verify_text_segments<F: FnMut(&VerificationIssue)>(
    ...
    let p = crate::index::text::index_prefix(crate::index::text::segments::SEGMENT, i.id);
    let end = prefix_end(&p);
    let source = run.reader.clone();
    ...
    visit(&source, &p, end.as_deref(), |key, value| {
        ...
        let postings = match crate::index::text::segments::decode(value) {
    ...
    for (term, sequence, frequency) in checks {
        let key = crate::index::text::posting_key(i.id, &term, sequence);
        ...
        if let Some((id, layout, row)) = primary_field(run, i, sequence)? {
            ...
            run.mismatch(
                IssueClass::Derived,
                &crate::index::text::segments::segment_key(i.id, &term, sequence),
                Some(i.id),
                Some(id),
...
fn verify_text_actual<F: FnMut(&VerificationIssue)>(run: &mut Run<F>, i: &IndexInfo) -> Result<()> {
    let p = crate::index::text::index_prefix(crate::index::text::POSTING, i.id);
    let end = prefix_end(&p);
    let source = run.reader.clone();
    ...
    let finish = |run: &mut Run<F>, term: &str| -> Result<()> {
        ...
        let key = crate::index::text::term_stats_key(i.id, term);
        let actual = run.read(&key)?;
    ...
    visit(&source, &p, end.as_deref(), |key, value| {
        ...
        // packed tier does.
        let frequency = match crate::index::text::decode_u32(value, "text posting") {
    ...
    let stats = crate::index::text::index_prefix(crate::index::text::TERM_STATS, i.id);
    ...
    visit(&source, &stats, stats_end.as_deref(), |key, value| {
        ...
        let stored = match crate::index::text::decode_count(value, "text term-stat value") {
            Ok(stored) => stored,
            Err(_) => {
                malformed(
                    run,
    ...
    let p = crate::index::text::index_prefix(crate::index::text::NORM, i.id);
    let end = prefix_end(&p);
    let source = run.reader.clone();
    ...
    visit(&source, &p, end.as_deref(), |key, value| {
        ...
        let len = match crate::index::text::decode_u32(value, "text norm") {
            Ok(len) => len,
            Err(_) => {
                malformed(
                    run,
    ...
    if let Some(bytes) = actual.as_deref() {
        if crate::index::text::decode_corpus(bytes).is_err() {
            malformed(
                run,
                IssueClass::Derived,
                &key,
...
fn verify_graph<F: FnMut(&VerificationIssue)>(
    ...
    let graph_header = if !enabled {
        for tag in [
            ...
            crate::index::graph::NAME_DESCRIPTOR,
            crate::index::graph::NAME_LOOKUP,
        ...
        for copy in 0..3u8 {
            let key = crate::index::graph::graph_header_key(copy);
            match run.read(&key)? {
                None => run.issue(VerificationIssue {
                    class: IssueClass::Catalog,
                    kind: IssueKind::Missing,
                ...
                Some(bytes) => match crate::index::graph::decode_graph_header(&bytes) {
                    Ok(candidate) if candidate == header => {}
        ...
        visit(
            &source,
            &[crate::index::graph::NAME_LOOKUP],
            Some(&tag_end(crate::index::graph::NAME_LOOKUP)),
            |key, value| {
                run.row(true)?;
                ...
                let descriptor = crate::index::graph::read_name(|k| run.read(k), key[1], id)?;
                if descriptor.name != name {
                    return Err(corrupt("graph lookup/descriptor mismatch"));
                }
                for copy in 0..3u8 {
                    let replica = crate::index::graph::name_descriptor_key(key[1], copy, id);
                    match run.read(&replica)? {
                        None => run.issue(VerificationIssue {
                            class: IssueClass::Catalog,
                            kind: IssueKind::Missing,
                        ...
                        Some(bytes) => match crate::index::graph::decode_name(&bytes) {
        ...
        if counts != [header.type_count, header.context_count] {
            run.issue(VerificationIssue {
                class: IssueClass::Catalog,
                kind: IssueKind::Mismatch,
                key: vec![crate::index::graph::GRAPH_HEADER],
                index: None,
                entity: None,
        ...
        visit(
            &source,
            &[crate::index::graph::NAME_DESCRIPTOR],
            Some(&tag_end(crate::index::graph::NAME_DESCRIPTOR)),
            |key, value| {
                run.row(true)?;
                ...
                let name = match crate::index::graph::decode_name(value) {
                    Ok(name) => name,
                    Err(Error::Unsupported(message)) => return Err(Error::Unsupported(message)),
                    Err(_) => {
                        run.issue(VerificationIssue {
                ...
                let lookup = crate::index::graph::name_lookup_key(name.kind, &name.name);
