//! The Tier-1 boolean atomics of `docs/lang/QL_CONTRACT.md` §3 -- `Any` (union),
...
#[path = "../../../lang/tests/sqlslice/fixture.rs"]
mod fixture;
...
/// right.
///
/// They are added HERE rather than in `lang/tests/sqlslice/fixture.rs` on purpose:
...
fn with_node<R>(node: &Node, f: &Fixture, k: &mut dyn FnMut(&QueryFilter<'_>) -> R) -> R {
    match node {
        ...
        Node::Radius(metres) => k(&QueryFilter::Point {
            index: f.index.loc,
            predicate: sekejap_core::collections::PointFilter::Radius {
...
fn a_leaf_with_no_set_is_refused_at_prepare() {
    ...
    let refusals: Vec<(&str, QueryFilter<'_>, &str)> = vec![
        (
            ...
            QueryFilter::Geometry {
                index: f.index.plot,
                predicate: sekejap_core::collections::GeometryFilter::DWithin {
                    geometry: sekejap_core::collections::Geom::Point(
                        centre.longitude(),
                        centre.latitude(),
                    ),
                    metres: 1_000.0,
    ...
    let bfs = sekejap_core::collections::BfsRequest {
        seed,
