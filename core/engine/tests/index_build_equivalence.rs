...
struct Seeded {
    people: sekejap_core::collections::CollectionId,
}
...
fn an_atomic_late_build_is_bounded_by_chunks_not_by_the_whole_index() {
    ...
    let hits = db
        .query_text(
            index,
            "flood",
            sekejap_core::collections::TextMatch::Any,
            5,
...
/// core/kernel/src/btree.rs measures the same quantity for the same reason).
///
...
///
/// Reads raw pages the way `dist/src/cli/collection_inspect.rs` does rather than
/// through the engine, so the shape it reports is the shape on disk and not
/// the shape the builder believes it wrote.
fn leaf_shape(store: &sekejap_core::pagewal::PageWalStore, tree: u16, no: u32, depth: usize,
...
fn seed_ticks(path: &std::path::Path) -> (Database, sekejap_core::collections::CollectionId) {
    let mut db = Database::create(path, cfg()).unwrap();
    let ticks = db
        .create_collection(
            "ticks",
