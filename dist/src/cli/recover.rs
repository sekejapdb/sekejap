...
fn bounded_read(path: &Path, max: u64) -> sekejap_core::Result<Vec<u8>> {
    use std::io::Read;
...
fn verify_schema(destination: &Path) -> sekejap_core::Result<Value> {
    let report: Value =
    ...
    let rows =
        sekejap_core::recovery::visit_raw_records(&destination.join("records.raw"), limit, |r| {
            if r.overflow_marker {
                return Err("complete row archive contains an unresolved marker".into());
            }
            Ok(())
        })?;
    let unresolved = sekejap_core::recovery::visit_raw_records(
    ...
    for entry in fs::read_dir(destination.join("layouts"))? {
        ...
        match path.extension().and_then(|s| s.to_str()) {
            Some("layout") | Some("candidate") => {
                let layout = sekejap_core::Layout::from_descriptor(&bounded_read(&path, 2081)?)?;
                let filename = path
                    .file_name()
                    .and_then(|s| s.to_str())
...
fn inspect(source: &Path) -> sekejap_core::Result<Value> {
...
fn run() -> sekejap_core::Result<()> {
    let args: Vec<String> = std::env::args().skip(1).collect();
    let result = match args.as_slice() {
        [command, source] if command == "inspect" => inspect(Path::new(source))?,
        [command, source, destination] if command == "schema" => {
            let report = sekejap_core::recovery::recover_typed_candidates(
                Path::new(source), Path::new(destination), &sekejap_core::recovery::DenseV3,
                sekejap_core::recovery::RecoveryOptions::default(),
            )?;
