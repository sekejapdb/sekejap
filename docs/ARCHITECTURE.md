...
`docs/GRAPH_CONTRACT.md` and `docs/QL_CONTRACT.md`. Module homes are
`docs/SOURCE_LAYOUT.md`. This document maps those contracts onto the modules
...
Contract: `docs/V2_COLLECTION_INTEGRATION.md`, `docs/FORMAT_V1.md`,
`docs/RECOVERY_CONTRACT.md`, `docs/FORMAT_FREEZE.md`.
...
before touching bytes (`docs/FORMAT_FREEZE.md`). Repair rebuilds a store from
...
(`src/store/recovery.rs`, `docs/RECOVERY_CONTRACT.md`).

### 1.2 B-tree keyspaces

Every user and catalog record is a cell in a kernel B-tree (`kernel/src/btree.rs`)
...
`docs/COLLECTIONS.md`). `open_snapshot` opens a read-only handle on the
...
Contract: `docs/COLLECTIONS.md`. Module: `src/collections/mod.rs`
...
Contract sections: `docs/QL_CONTRACT.md` (family hooks), `docs/SPATIAL_FUNCTIONS.md`
(point/geometry), `docs/GRAPH_CONTRACT.md` (graph). Directory:
...
Contract: `docs/SPATIAL_FUNCTIONS.md`. Key:
...
`Within`/`Contains` planar — `docs/QL_CONTRACT.md` §4.4 and
`docs/SPATIAL_FUNCTIONS.md`). Geometry postings do not certify the filter
without a row.
...
Contract: `docs/QL_CONTRACT.md` "Exact vector". Locator key:
...
Contract: `docs/QL_CONTRACT.md` "Approximate vector". Companion to exact, not
...
spellings are T2 aliases of this family (`docs/QL_CONTRACT.md` §4.5).
...
Contract: `docs/GRAPH_CONTRACT.md` §§1–3, 6. Feature bit `GRAPH_FEATURE=2`.
...
Contract: `docs/QL_CONTRACT.md` "Contract and public shape", "Execution
...
driver/filter is `docs/GRAPH_CONTRACT.md` §4.5.
...
(`docs/QL_CONTRACT.md` dialect deviation 3). `ScoreExpr` leaves: `Lit`,
...
without a filter; `docs/QL_CONTRACT.md` §6).

## 5. Graph: contexts, edges, traversal

Contract: `docs/GRAPH_CONTRACT.md`. Module: `src/index/graph/mod.rs`. Query
...
The laws are `CONTRACT.md` / `docs/FOUNDATION_TEST_STANDARD.md`. One line
...
| 3 Nothing fallible may delete | Write new page images, CRC, publish after FULL barrier; checkpoint only with every reader slot; failed repair leaves the source; cascade preflights then deletes pairs. | RESTRICT is the contract default (`docs/GRAPH_CONTRACT.md` §6.1) and is not the implemented delete. |
...
| 8 Compatibility | Additive feature bits (`GRAPH_FEATURE`, `TEXT_FEATURE`, `SEGMENT_FEATURE`, `GEOMETRY_FEATURE`, `INDEX_TREE_FEATURE`, …); unknown bits refuse; old files without a bit open with the prior encoding. | `L8-COMPAT` remains PENDING (`docs/FOUNDATION_TEST_STANDARD.md`); element identity and typed edge properties are specified as future additive bits. |
