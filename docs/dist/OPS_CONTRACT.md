# Operations contract — sekejap 0.17

Companion to `docs/lang/QL_CONTRACT.md` (what the language is)
...
The reason it exists: when `docs/lang/E3_PARITY.md` was first written it
counted 33 capabilities of THE PRIOR RELEASE -- the engine this tree replaces,
whose sources are on branch `e1` and which the rest of this document calls
"the prior release" -- that the contracts did not mention at any tier, and the
largest single cluster was this one -- a long-running service form with
publish semantics, a statement timeout, a cancellation handle and a
commit-time change feed. The engine had every ingredient (`open_snapshot`,
`Error::Cancelled`, the `WorkMeter` cancellation closure, `io_counters`,
`pool_counters`, `checkpoint`) and no contract that said what they added up
to. That was a missing document, not a missing tier row. This is the
document; ten of its fourteen sections are now built, and the status table
below says which.

Tiers are `docs/lang/QL_CONTRACT.md`'s: **T1** compiles to an atomic that
exists in this tree AND a named test proves it; **T2** names the atomic and is
not built; **T3** is refused with the reason in the error text and never
emulated.

**BUILT / NOT BUILT, per section, in one line each.** Every BUILT section
names its test file below its heading.

| § | surface | status |
|---|---|---|
| 1 | Service mode | **BUILT** — `dist/src/service/mod.rs`, `dist/tests/service.rs` |
| 2 | `publish()` and the staleness window | **BUILT** — `dist/src/service/mod.rs`, `dist/src/service/snapshot.rs`, `dist/tests/service.rs` |
| 3 | Statement timeout | **BUILT** — `core/engine/src/query/mod.rs` (`QueryBudget::deadline`), `dist/src/service/mod.rs`, `dist/tests/service.rs` |
| 4 | Interrupt handle | **BUILT** — `dist/src/service/interrupt.rs`, `dist/tests/service.rs` |
| 5 | Change notifications | **BUILT** — `dist/src/service/changes.rs`, `dist/tests/service.rs` |
| 6.1 | `SHOW STATUS` | **NOT BUILT** (T2). `SHOW STATUS` is not a spelling `lang/src/parser/catalog.rs` reads |
| 6.2 | `SHOW STORAGE` | **NOT BUILT** as a statement (T2). Its FILE half IS built as a call: `Database::storage_bytes` -> `Db::storage()` -> `sekejap_storage`, pinned by `dist/ffi/tests/abi.rs::storage_answers_the_two_files_and_their_total_and_a_checkpoint_reports_whether_it_folded`. The per-keyspace attribution is the scan, and it is the part that is missing |
| 6.3 | `stats`, `memory_report`, `trim_memory` | **NOT BUILT** (T2). `trim_memory` ships as a REFUSAL by name in both surfaces: `dist/ffi/tests/abi.rs::the_four_calls_sekejap_has_no_atomic_for_refuse_by_name_and_answer_nothing_else` |
| 7 | Bulk load | **BUILT** — `core/engine/src/collections/write_set.rs` (`begin_bulk`/`end_bulk`), SQL `BEGIN BULK`/`END BULK`; `core/engine/tests/write_where.rs`, `lang/tests/sql_dml.rs`. The two BATCHED entry points are NOT adopted, and §7 says why |
| 8 | `write_trace` | **NOT BUILT** (T2). There is no `write_trace` in this tree |
| 9.1 | `statement_timeout` as a GUC | **BUILT** — `dist/src/pg/`, `dist/tests/pg_wire.rs`, `dist/tests/pg_server.rs` |
| 9.2 | `CancelRequest` as the interrupt handle | **BUILT** — `dist/src/pg/`, `dist/tests/pg_wire.rs`, `dist/tests/pg_server.rs` |
| 9.3 | `LISTEN` / `NOTIFY` as the change feed's surface | **BUILT** — `dist/src/pg/`, `dist/tests/pg_wire.rs`, `dist/tests/pg_server.rs` |
| 9.4 | What the wire does not get | **BUILT** as a set of named refusals — `dist/tests/pg_wire.rs::a_refused_construct_arrives_as_0a000_with_the_contracts_named_reason` |

Ten of the fourteen are built. The four that are not are one group -- the
introspection and tracing group: §6.1, §6.2 as a statement, §6.3 and §8.
...
| # | Surface | prior release (file:line) | sekejap ingredient | Tier | Law |
|---|---|---|---|---|---|
...
| 6 | Introspection: `SHOW STATUS`, `SHOW STORAGE`, `stats`, `memory_report`, `trim_memory` | `src/sql.rs:8132-8135`, `src/exec.rs:463-540`, `src/db.rs:10177`, `:11726`, `:11763` | `storage_bytes` (`core/engine/src/collections/mod.rs:1110`), `io_counters`, `pool_counters`, `tracked_pages` | T2, except the FILE half of §6.2, which ships as `Db::storage()` / `sekejap_storage` | L4 |
| 7 | Bulk load: `begin_bulk` / `put_value_bulk` / `link_many` | `src/db.rs:11913-11997` | `commit` as the durability point; `put`, `put_edge` | **DONE** — `core/engine/src/collections/write_set.rs` (`begin_bulk`, `end_bulk`), SQL `BEGIN BULK` / `END BULK`; `Db::put_many` and `Database::link_many` for the two batched calls sekejap does adopt | L2, L3 |
...
§7 is BUILT, in `core/engine/src/collections/write_set.rs` with the SQL
spelling `BEGIN BULK` / `END BULK`. §9.1-§9.4 are BUILT on the wire, in
`dist/src/pg/`. §6 and §8 are still T2. The three named T3 exceptions inside
§1, §3 and §5 stand, and none of them is emulated.
...
**BUILT.** `dist/src/service/mod.rs` (`ServiceDatabase`, `open`, `writer`, `try_writer`, `reader`, `open_reader`, `close`), `dist/src/service/snapshot.rs`. Tests: `dist/tests/service.rs::a_reader_keeps_its_old_snapshot_while_a_writer_commits_and_a_later_reader_sees_the_commit`, `::a_second_writer_is_refused_by_name_while_the_first_holds_it_and_served_once_it_is_dropped`, `::a_second_service_on_one_directory_is_refused_by_the_stores_own_writer_lock`, `::close_discards_uncommitted_work_and_releases_the_reader_slot`, `::transaction_control_sql_is_refused_through_the_writer_guard`.

**What the prior release does.** `CoreDB::open_as_service(dir)`
(`src/service.rs:174-186`) returns a `ServiceDb`: one process, a `Mutex<Db>`
writer, and an `RwLock<Arc<Db>>` published read view. Writes serialise on the
writer lock; reads clone the `Arc` under a momentary read lock and then run
entirely lock-free on a pinned read-only handle, so a slow write, a bulk load
or a compaction never stalls a query (`src/service.rs:8-14`). The type is
`Send + Sync` by construction and is meant to be shared behind an `Arc` rather
than wrapped in the caller's own lock (`src/service.rs:36-39`, `:191-194`).
`open()`
stays the embedded form that starts and stops with an app; this is the form a
server, an agent or a gateway keeps alive.

**sekejap's ingredient.** `Database::open_snapshot(path, config)`
...
blocks the writer's checkpoint. `docs/core/ARCHITECTURE.md` §1.3 already states
the single-writer rule. What is missing is the object that owns both halves.
...
else. sekejap's slot count is persisted and bounded (`readers` in
`ResourceLimits`, `docs/core/ARCHITECTURE.md` §2), so the service must refuse
to mint a snapshot past that bound rather than block: **a service that blocks a
...
**BUILT.** `dist/src/service/mod.rs` (`publish_now`, `publish_interval`, `mark_dirty_and_maybe_publish`), `dist/src/service/snapshot.rs` (`PUBLISH_INTERVAL_DEFAULT`). Tests: `dist/tests/service.rs::a_commit_becomes_visible_within_the_publish_interval_plus_one_snapshot_open`, `::publish_now_swaps_the_view_in_one_snapshot_open_and_leaves_the_old_one_intact`.

**What the prior release does.** After a commit the writer republishes at most
once per `publish_interval` (default 100 ms, `src/service.rs:186`), paying the
mint cost itself so readers never do, and only when a commit actually happened
— `dirty` is set by a commit and cleared by a publish
(`src/service.rs:136-151`). A read is therefore stale by at most one interval
plus one mint. For read-your-own-writes, `ServiceDb::publish()` mints
synchronously (`src/service.rs:118-133`). A failed mint leaves readers on the
view they already had, which is Law 3's shape at the API: the old photo is
dropped only once the new one is in hand (`src/service.rs:143-146`). The prior
release's own note records the predecessor engine shipping this default-on and
undocumented, with 53% of same-thread write-then-read cycles missing the write
at a 5 ms interval — the contract here is the same trade, written down.

**sekejap's ingredient, and where it is cheaper.** The prior release's
`publish` must `publish_generation()` — commit **and checkpoint** — before
minting, because a snapshot there opens the newest published *generation* and
commits alone still live in the WAL (`src/service.rs:120-122`). sekejap does
not have that constraint: `commit` publishes, and `open_snapshot` reads the
newest published transaction through the committed-WAL overlay
(`docs/core/ARCHITECTURE.md` §1.3, `src/collections/mod.rs:710-716`).
**sekejap's publish barrier is therefore one checkpoint cheaper than the prior
release's**: mint a fresh snapshot handle, swap it in, done. Checkpointing
stays what it is — a separate, deferrable fold — and is not on the
...
matching the prior release so a migrating caller's timing does not change under
it. No measurement is claimed here: the prior release's mint costs (~12 ms at
1M rows, ~56 ms at 48M) are its own numbers on its own path, and sekejap's
snapshot open has not been measured for this document. The first item of work
is to measure it and put the metric in this section.
...
**The metric this section asked for, measured.** On a 4,000-row corpus, Apple
M-series laptop, release build, `IoMode::Buffered` / `SyncMode::Full`, four
runs: one snapshot open is **0.9 ms to 6.7 ms**, and one `publish_now` swap is
**0.9 ms to 2.0 ms** -- that open plus an `Arc` store under a write lock, and
nothing else. A commit made just after a publication became visible to a new
reader **100.3 ms to 108.5 ms** later, measured from the commit, against a
stated window of 100 ms + one open. The prior release's mint costs on its own
path were ~12 ms at 1M rows and ~56 ms at 48M; sekejap's is one checkpoint
cheaper by construction, and these are the first sekejap numbers for it. The
open cost grows with the corpus and these are 4,000-row numbers: what is fixed
is the SHAPE of the window, not its second term. Test:
`a_commit_becomes_visible_within_the_publish_interval_plus_one_snapshot_open`,
`publish_now_swaps_the_view_in_one_snapshot_open_and_leaves_the_old_one_intact`.

**The second deviation.** The prior release's writer pays the mint so a reader
never does. This build starts no background thread, so a reader that finds the
view dirty and older than the interval pays it; the writer still pays it
whenever another commit follows within the interval. Without that, a database
whose last commit is its last write would serve a view that never refreshes.

## 3. Statement timeout

**BUILT.** `core/engine/src/query/mod.rs` (`QueryBudget::deadline`, `with_deadline`, `WorkResource::Deadline`, `DEADLINE_POLL_CHARGES` = 1,024, `check_deadline`) and `dist/src/service/mod.rs` (`set_statement_timeout`). Tests: `dist/tests/service.rs::a_statement_past_its_deadline_is_refused_naming_deadline_the_elapsed_micros_and_the_work_so_far`, `::a_work_bound_still_refuses_by_its_own_resource_while_a_deadline_is_also_set`.

**What the prior release does.** `Db::set_statement_timeout(Option<Duration>)`
...
**sekejap's ingredient.** `WorkMeter::charge` (`src/query/mod.rs:581`) already
calls `check_cancelled` (`:528`) before charging any resource, and every walk
charges — candidates, primary reads, postings per family, graph edges and
visited nodes, key postings, output bytes. **The check points already exist and
are already at the right density**: one per unit of work, on every driver.

**The atomic to build.** A `deadline: Option<Instant>` beside the cancellation
closure in `WorkMeter`, tested in `check_cancelled` on a counted interval of
charges (the prior release's 1024 is the precedent; the interval is a stated
...
(`QueryBudget::with_deadline`, so no caller restates the work bounds it already
chose), `:647` (`WorkResource::Deadline`), `:717` (`DEADLINE_POLL_CHARGES` =
**1,024 charges**, the prior release's precedent), `:762` / `:778`
(`check_cancelled` -> `check_deadline`). The setter is
...
**BUILT.** `dist/src/service/interrupt.rs` (`InterruptHandle`) and `dist/src/service/mod.rs` (`interrupt_handle`, `cancel`, `clear_interrupt`, and `scan`, which threads it into every `next_page`). Tests: `dist/tests/service.rs::a_cancel_from_another_thread_stops_a_long_scan_and_clearing_it_restores_the_reader`, `::a_standing_cancel_does_not_stop_the_writer_from_committing`.

**What the prior release does.** `InterruptHandle` (`src/db.rs:325-337`) is a
cloneable `Arc<AtomicBool>`; `Db::interrupt_handle()` (`src/db.rs:5014-5018`)
hands one out, `cancel()` sets it, `is_cancelled()` reads it, and
`Db::clear_interrupt()` (`src/db.rs:5021-5023`) clears it. The shape is
`sqlite3_interrupt`'s. The semantics the prior release states and sekejap
should keep: cancellation is **per handle and therefore per snapshot** —
cancelling stops every query in flight on that view, which for a shared read
snapshot is all of its readers; a timeout is per statement and needs no
clearing, while an explicit cancel is **sticky** until cleared. The running
query stops at its next check point and returns an error naming the
cancellation.

**sekejap's ingredient.** More of this exists than anywhere else in this document.
...
per charge, the same cost the prior release pays.
...
**BUILT.** `dist/src/service/changes.rs` (`ChangeEvent`, `CHANGE_QUEUE_BOUND` = 256, `CHANGE_KEY_CAP` = 1,024, `deliver`) and `dist/src/service/mod.rs` (`WriterGuard::commit`, the single emission point). Tests: `dist/tests/service.rs::exactly_one_event_per_committed_batch_and_the_event_never_precedes_durability`, `::a_rolled_back_batch_and_a_dropped_guard_fire_no_event_and_leave_no_rows`, `::a_subscriber_that_stops_draining_drops_events_past_the_stated_bound_and_counts_them`, `::the_key_list_stops_at_the_stated_cap_and_the_event_reports_the_total`, `::unsubscribing_stops_the_feed_and_an_unlistened_commit_records_nothing`.

**What the prior release does.** `Db::subscribe_changes(callback) -> u64` and
...
and the **edge type** labels that were linked or unlinked. The delivery rule is
exact and is the whole value of the primitive: **one event per committed
...
immediately on an empty listener list (`src/db.rs:5049-5062`). The prior
release names this what it is: the primitive a reactive `.watch()` is built on.
A listener learns *which* collections, keys and edge types moved, then re-runs
whatever query it cares about.

**sekejap's ingredient.** `Database::commit` (`src/collections/mod.rs:1605`) is
the single point every mutation passes through to become durable, and it is
...
**The Law — and the correction sekejap must make.** **L6** for the feed itself: an
...
**L1 for its bound, and this is where sekejap must not copy it.** The prior
release's `ChangeEvent.keys` is a `Vec<String>` that accumulates every changed
key in the batch, deduplicated only against the immediately preceding entry
...
Law 1 violation that the prior release's own bulk-load path (§7) makes
reachable in one call. sekejap's contract bounds it: **the collections and
edge-type lists are carried in full** (both are bounded by the catalog, which
is bounded by the schema), and **the key list is carried up to a stated cap,
after which the event reports `keys_truncated` with the total count and the key
list is dropped**. A listener past the cap does what a listener is supposed to
do anyway — re-run its query against the collections it was told about. The cap
is a constant fixed at open, in the same family as the plan cache's three
ceilings.
...
**NOT BUILT** (T2). `SHOW` reads `TABLES`, `EDGES`, `INDEXES [ON t]`, `CREATE TABLE t`, a collection name or a client setting (`lang/src/parser/catalog.rs`); `STATUS` is none of those, so the statement is refused naming what `SHOW` does take.

**What the prior release does.** `src/exec.rs:519-540` returns one `(name,
value)` row per fact: format version, generation, mode (`snapshot` or
`embedded`), sync mode, collection count, node count, edge count, data bytes,
WAL bytes, and a query counter.

**sekejap's ingredient.** Format version and the create-feature bits live in
...
**NOT BUILT** as a statement (T2). The FILE half of the answer is built as a CALL: `Database::storage_bytes` (`core/engine/src/collections/mod.rs:1110`) behind `Db::storage()` (`dist/rust/src/db.rs:743`) and `sekejap_storage` (`dist/ffi/src/lib.rs:1742`), pinned by `dist/ffi/tests/abi.rs::storage_answers_the_two_files_and_their_total_and_a_checkpoint_reports_whether_it_folded`. What is missing is the per-keyspace attribution, which is the scan.

**What the prior release does.** `src/exec.rs:463-518`: one full store scan
attributing live key + value bytes and row counts to each keyspace tag —
catalog, node payloads, membership, edges, reverse edges, vectors, external
keys, properties, vector codes, text postings, text metadata, spatial grid,
geometry, vector nav graph, SQL metadata, field indexes, search slots — then
the `data` and `wal` file sizes. The prior release's own comment prices it
honestly: *an administrative command, priced like one*.

**sekejap's ingredient.** The same shape: sekejap's keyspaces are tag-prefixed
...
**NOT BUILT** (T2). `trim_memory` is shipped as a REFUSAL BY NAME rather than as a no-op, in both surfaces: `docs/dist/RUST_API.md` §7 lists it as absent, and `sekejap_trim_memory` returns its sentinel with `SekejapStatus_Refused` (`dist/ffi/tests/abi.rs::the_four_calls_sekejap_has_no_atomic_for_refuse_by_name_and_answer_nothing_else`). That is this section's own rule applied to itself: a report with nothing to report is a refusal, not a zero.

**What the prior release does.** `stats()` (`src/db.rs:10177-10198`) is the
struct `SHOW STATUS` reads, with counts computed, not cached — the prior
release's doc comment says to call it for a report, not in a loop.
`memory_report()` (`src/db.rs:11763-11806`) returns per-structure resident
bytes and is written with unusual care: it names only structures that exist,
refusing to report absent structures as `0` because *"reporting them as 0 would
name the wrong structure with the authority of a measurement"*. The kernel
buffer pool appears as the arena it reserves at open, labelled a ceiling and
not a residency. `trim_memory()` (`src/db.rs:11726-11747`) shrinks the
registries and returns free arenas to the OS, and is explicitly safe: it drops
no data and no index, so every query answers the same after it as before.

**sekejap's ingredient.** `pool_counters` (`src/collections/mod.rs:892`) gives
hits, misses, evictions and clock-sweep steps, with the doc comment already
making the distinction a scan budget needs — a miss is a pread and a checksum,
a hit is a hash lookup. `tag_hint_stats` (`:940`) and `io_counters` (`:947`)
are the rest.

**The atomic to build.** `Database::memory_report()` over sekejap's actually
resident structures, and `trim_memory()` over the caches that can shrink
(`index_cache`, the catalog and layout caches).

**What makes sekejap's report short, and why that is the point.** sekejap holds
nothing proportional to rows: rows are in the tree and are served through a
fixed-size buffer pool reserved at open. So the honest sekejap report is the
pool arena (a ceiling, labelled), the index and layout caches (proportional to
collections and declared indexes), and the reader-slot state. **That short list
is Law 1 being true, stated as a measurement.** The prior release's refusal to
report absent structures as zero is adopted verbatim as a rule of this
contract.
...
**BUILT**, in sekejap's own spelling. `Database::begin_bulk` / `end_bulk`
(`core/engine/src/collections/write_set.rs`) and the SQL `BEGIN BULK` / `END
BULK` (`lang/src/compile/plan.rs`, `docs/lang/QL_CONTRACT.md` §2). Tests:
`core/engine/tests/write_where.rs::a_bulk_scope_nests_and_only_the_outermost_close_commits`,
...
`lang/tests/sql_dml_adversarial.rs::a_refused_predicated_write_inside_a_bulk_scope_leaves_the_counter_where_it_was`,
`::a_snapshot_opened_inside_a_bulk_scope_sees_nothing_of_the_batch`. The
BATCHED entry points `put_value_bulk` and `link_many` are NOT adopted as the
prior release spells them: a loop inside the scope IS the batch, because `put`
/ `delete` / `put_edge` already write with no durability point of their own.
The one batched call that ships is `Db::put_many` / `sekejap_put_many` -- N
puts into ONE collection under one commit
(`dist/ffi/tests/abi.rs::put_many_writes_one_batch_and_answers_how_many_rows_it_wrote`)
-- and `Database::link_many` exists in core
(`core/engine/src/index/graph/mod.rs:1585`) for the
one-edge-per-distinct-endpoint saving.

**What the prior release does.** `begin_bulk()` / `end_bulk()`
(`src/db.rs:11913-11938`) open a scope that defers the durability point to the
matching close, so a stream of writes costs one commit instead of one per
write; scopes nest and only the outermost close commits, and unbalanced calls
are absorbed rather than panicking because the call arrives from FFI.
`put_value_bulk(rows)` (`src/db.rs:11951-11977`) is the node form and
`link_many(edges)` (`src/db.rs:11987-11997`) the edge form, both nesting-safe
so neither cuts an outer batch short. The prior release is precise about what
is won: not a skipped fsync — it has no per-write fsync — but the batching of
the durability point.

**sekejap's ingredient.** The same fact holds and is easier to state: sekejap's
...
without one. A bulk load in sekejap is already "many puts, then one commit"; what
...
**Where sekejap is stronger, stated as a deviation.** The prior release's
`put_value_bulk` aborts at a malformed row "leaving the rows before it stored".
sekejap has `rollback` (`src/collections/mod.rs:1628`), which discards the
...
migrating from the prior release must expect the stronger guarantee, not the
weaker one.
...
Three deviations from the prior release, each stated rather than inherited:

* **No batched entry points.** The prior release needs `put_value_bulk(rows)` and
  `link_many(edges)` because its scope is nesting-*unsafe* without them. sekejap's
...
* **An unbalanced `end_bulk` is an error.** The prior release absorbs it because the call
...
**NOT BUILT** (T2). There is no `write_trace` in this tree, feature-gated or otherwise. Law 4 is served today by the counters, not by attributed time: `io_counters` and `pool_counters` on the store, and the `QueryWork` counters `EXPLAIN` prints.

**What the prior release does.** `src/write_trace.rs:1-22` is feature-gated
end-to-end timing for one transaction payload operation, broken into the phases
that actually cost: identity parse, prepare, reads of the old row, the primary
put (itself a nested kernel `PutTrace`), bookkeeping, the membership probe, the
label write, membership metadata, index maintenance, the collection catalog,
page CRC (with its count), and the number of caller-side value copies made
before the kernel sees the bytes. State is thread-local and the module is
feature-gated, so the timers cost nothing when off.

**sekejap's ingredient.** `io_counters` (`src/collections/mod.rs:947`) already
...
already reports the per-keyspace append hints. sekejap counts the I/O; it does not
yet attribute the time.

**The atomic to build.** A `cfg(feature = "write-trace")` thread-local phase
timer on the write path, with sekejap's own phase list: key mapping, row encode,
...
**BUILT.** `dist/src/pg/` is SANS-IO (`pg::Connection::feed(bytes) -> bytes` touches no socket), `pg::server` is the `std::net` adapter and `dist/src/cli/pg_server.rs` is the `sekejap-pg` binary. The contract is `docs/dist/WIRE_CONTRACT.md`; the tests are `dist/tests/pg_wire.rs` (21) and `dist/tests/pg_server.rs` (9, one of them against a real `psql` when one is installed).
...
**§9.1-§9.4 are BUILT**, in `dist/src/pg/` over the §1-§5 service, with the
whole surface written down in `docs/dist/WIRE_CONTRACT.md`:

| § | Surface | Where |
|---|---|---|
| 9.1 | `SET`/`SHOW statement_timeout` -> `ServiceDatabase::set_statement_timeout` | `dist/src/pg/connection.rs` (`set_statement`, `set_statement_timeout`, `parse_timeout_micros`), `WIRE_CONTRACT.md` §2 |
| 9.2 | `BackendKeyData` + `CancelRequest` -> the interrupt, `57014` for both it and §3 | `dist/src/pg/connection.rs` (`BackendKey`, `CancelToken`, `cancel_request`), `dist/src/pg/server.rs` (`Backends::cancel`), `dist/src/pg/types.rs` (`QUERY_CANCELED`), `WIRE_CONTRACT.md` §4, §8 |
| 9.3 | `LISTEN`/`UNLISTEN` -> one `NotificationResponse` per listening channel per COMMITTED batch; `NOTIFY` refused by name | `dist/src/pg/connection.rs` (`drain_changes`, `notification_payload`, `NOTIFY_REFUSAL`), `WIRE_CONTRACT.md` §5 |
| 9.4 | No TLS, no auth beyond trust, no `COPY`; the catalog views are `sekejap_lang`'s (docs/dist/PG_SURFACE.md) | `dist/src/pg/server.rs` (`bind` refuses a non-loopback address), `WIRE_CONTRACT.md` §9 |

Three consequences the build settled and this document now states:

* §9.1's GUC sets the SERVICE's bound, because §3's bound is per service
  exactly as §4's handle is. A session that sets it sets it for the server.
* §9.2 needs TWO handles, not one: a `CancelRequest` names ONE backend, so
  the wire adds a per-connection cancel flag beside §4's per-service handle
  and ORs them into the same closure. The per-connection one is cleared at
  the next `ReadyForQuery` -- a cancel stops the statement in flight and
  nothing after it -- and §4's stays sticky, because that one is the
  operator's.
* §9.3's `NOTIFY` is a REFUSAL, not a surface: the feed is the source of
  notifications and there is no second queue for a client-issued `NOTIFY` to
  write into.

The tests are `dist/tests/pg_wire.rs` (byte level, sans-IO) and
`dist/tests/pg_server.rs` (the `sekejap-pg` binary, driven by the `postgres`
crate and by `psql`).

### 9.1 `statement_timeout` as a GUC

**BUILT.** Tests: `dist/tests/pg_server.rs::a_statement_timeout_reaches_the_client_as_57014`; `dist/tests/pg_wire.rs::a_statement_that_outruns_the_timeout_arrives_as_57014_with_the_elapsed_microseconds`.

`SET statement_timeout = '5s'`, `SET LOCAL statement_timeout = '5s'`, and the
same key in a connection's startup parameters. sekejap already parses
...
**BUILT.** Tests: `dist/tests/pg_server.rs::a_cancel_from_a_second_client_stops_the_statement_in_flight_with_57014`; `dist/tests/pg_wire.rs::a_cancel_request_names_the_backend_it_wants_stopped_and_then_closes`, `::a_cancelled_statement_arrives_as_57014`. The two are told apart by the MESSAGE, not the code: `dist/src/pg/types.rs::CANCELLED_MESSAGE` and `::deadline_message`.
...
**BUILT.** Tests: `dist/tests/pg_server.rs::listen_delivers_a_notification_after_a_commit_made_on_another_connection`; `dist/tests/pg_wire.rs::listen_delivers_one_notification_per_listening_channel_after_each_commit_and_none_before`, `::a_client_issued_notify_is_refused_by_name_rather_than_emulated`.
...
**BUILT** as a set of named refusals, which is what this section promises. Tests: `dist/tests/pg_wire.rs::a_refused_construct_arrives_as_0a000_with_the_contracts_named_reason`; `dist/tests/pg_server.rs::a_refused_construct_reaches_the_client_as_0a000_with_the_contracts_reason`.
...
4. ~~§5 the change feed~~ — **DONE**, with both of its L1 bounds decided and
   ...
   event).
5. ~~§7 the bulk-load scope~~ — **DONE**, as `BEGIN BULK` / `END BULK`, with
   the two batched entry points named as NOT adopted and the reason written
   in §7.
6. ~~p3-wire, which consumes §3, §4 and §5 in that order (§9)~~ — **DONE**:
   `dist/src/pg/`, `docs/dist/WIRE_CONTRACT.md`, `dist/tests/pg_wire.rs`,
   `dist/tests/pg_server.rs`.

What is left, in order:

7. §6.1 `SHOW STATUS` and §6.3 `stats` / `memory_report` / `trim_memory`:
   O(1) reads over counters that exist. `trim_memory` already ships as a
   refusal by name, so what is left there is the REPORT.
8. §6.2 `SHOW STORAGE`: the per-keyspace attribution is a scan, so it waits
   for the ones that are not. Its file half already answers as a call.
9. §8 `write_trace`, feature-gated, last.
