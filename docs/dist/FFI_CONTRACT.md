# FFI contract — the 0.16 `sekejap.h` mapped onto 0.17

Read-only map of the 0.16 C ABI (`wrappers/c/include/sekejap.h`,
generated from `wrappers/c/src/lib.rs` by cbindgen) onto this tree. "0.16"
throughout this document means the PRIOR ENGINE this one replaces: a separate
storage design with its own C surface, whose sources are on branch `e1`. That
header is 41 `extern "C"` functions (counted from the declarations in
`sekejap.h:47-328`), not ~72.

**BUILT / NOT BUILT, per section.** This document is a MAP, not a spec, and
the thing it maps onto has since shipped. Read it accordingly:

| § | what it is | status |
|---|---|---|
| 0 | the shared ownership and error rules | **BUILT** and kept, with ONE stated change: `sekejap_last_error` is thread-local now, not handle-local (§8, row 28). Pinned by `dist/ffi/tests/abi.rs::every_failure_returns_its_sentinel_and_leaves_a_message_and_a_code_on_this_thread`, `::one_database_handle_serves_two_threads_at_once_and_each_keeps_its_own_error_slot`, `::every_returned_string_is_freed_by_the_library_and_every_close_is_null_safe` |
| 1 | every function of the 0.16 header, with the sekejap call it needs | **HISTORICAL.** Its "status" column (READY NOW / READY AFTER D1 / LATER / REFUSED) is the plan as it stood before the ABI was written. What each symbol ACTUALLY became is §8 |
| 2 | the result-set encoding | **BUILT** — the JSON-array envelope, with §2.2's one documented change (each row object is keyed by COLUMN NAME). `dist/ffi/tests/abi.rs::a_query_answers_a_json_array_of_objects_keyed_by_column_name`, `::a_paged_query_hands_back_the_same_rows_as_one_query_in_pages_of_the_stated_size` |
| 3 | threading: `engine_*` vs `ServiceDatabase` | **BUILT, differently.** There is no second handle type: `sekejap::Db` is `Send + Sync`, so one `SekejapDb*` is shared across threads and service mode is the same handle opened another way. §8, rows 31-40. `dist/ffi/tests/abi.rs::one_database_handle_serves_two_threads_at_once_and_each_keeps_its_own_error_slot`, `::the_service_calls_are_refused_by_name_on_a_handle_opened_in_single_mode`, `::a_service_handle_answers_the_timeout_the_cancel_and_one_change_event_per_commit` |
| 4 | the proposed crate layout and cbindgen | **BUILT, at a different path.** §4.1 proposed `dist/src/ffi/` inside `sekejap-dist`; what shipped is a SEPARATE crate, `dist/ffi` (`sekejap-capi`, lib name `sekejap`), over the published `sekejap` crate rather than over `Database` directly -- see the note at the head of §4. §4.3's wrapper table is superseded by §8.1 |
| 5 | what must not be reproduced, and the names to keep | **BUILT** as written, except where §8 records a deliberate change (the slug argument, the second handle type, three symbols that became refusals) |
| 6 | the status counts of the 41 | **HISTORICAL.** Its "READY AFTER D1" rows all landed; §8 is the current column |
| 7 | the Rust facade column | **BUILT** — `sekejap` 0.17.0, `dist/rust`, `docs/dist/RUST_API.md`, `dist/rust/tests/api.rs` (23 tests) |
| 8 | the 0.17 header column | **BUILT** — `libsekejap` 0.17.0, `dist/ffi`, header `dist/ffi/include/sekejap.h` (59 functions), `docs/dist/C_ABI.md`, `dist/ffi/tests/abi.rs` (17 tests), and `cd dist/ffi && make check` compiling and running `examples/smoke.c`. **This is the section that describes what exists.** |

The 0.17 C ABI that SHIPPED is `docs/dist/C_ABI.md`, built in `dist/ffi`.
Which 0.16 symbol became which of its 59 functions -- or was dropped, and why
-- is §8 below. The 0.16 sources this document cites are on branch `e1`
(`git show e1:dist/bindings/wrappers/c/src/lib.rs`).
...
| status | meaning |
|---|---|
| READY NOW | sekejap call exists at HEAD; the C function can be a thin wrapper |
...
Failure sentinels are the 0.16 header's: `NULL` for pointers, `-1` for integers
...
Copied from the 0.16 header and `lib.rs` design comment. sekejap's FFI must
keep them so existing C callers stay valid.

| rule | 0.16 | sekejap FFI |
|---|---|---|
...
/ `create_*` (`lang/src/lib.rs:522-531`). sekejap `put` writes the working
tree; `commit` is the durability barrier (`collections/mod.rs:1580-1586`,
`:1796-1808`). The prior engine's `put` / `execute` append the WAL before
mutating (`src/lib.rs:5316-5328`, `:9411-9413`). To keep the C ABI, the FFI
handle must auto-commit after a mutating call unless the caller has an open
`BEGIN`…`COMMIT` (sekejap `BEGIN` is a notice that the writer is already in a
transaction; `COMMIT` calls `Database::commit` —
`lang/src/compile/plan.rs:837-845`).

Slug convention on the C side is `"collection/key"` (`sekejap.h:84`, `:166`).
sekejap splits that into `Database::collection(name)` + `put`/`get`/`delete(c,
key)` (`collections/mod.rs:1346-1357`, `:1580`, `:1677`, `:1763`). The internal
key field is `__e4_key` (`collections/mod.rs:83`); SQL spells it `_key`
...
| # | C signature (`sekejap.h`) | what 0.16 does | sekejap call(s) | status | ownership / error |
|---|---|---|---|---|---|
| 1 | `SekejapDb *sekejap_open(const char *path)` `:47` | `CoreDB::open` (`src/lib.rs:2547-2549`, via `open_impl` `lib.rs:168-201`) | If the directory has no `data`/`writer.lock`, `Database::create(path, Config)`; else `Database::open(path, Config)` (`collections/mod.rs:798-844`). `Config` is `kernel::store::Config { budget_bytes, io, sync }` (`core/kernel/src/store.rs:67-68`). | READY NOW | returns boxed handle or `NULL`. Open failure does not set `last_error` in 0.16 (`lib.rs:193-199` drops the `io::Error`). sekejap FFI should set a thread-unrelated open-error string **or** keep 0.16's silent `NULL`; pick one and document it. Recommendation: set a process-level last-open-error readable via a static, because `sekejap_last_error` needs a handle (`sekejap.h:249`). |
| 2 | `SekejapDb *sekejap_open_paged(const char *path)` `:54` | `CoreDB::open_paged` — mmap topology (`src/lib.rs:2559-2561`, `lib.rs:178-180`) | sekejap has one store: the page-WAL. `Database::open` / `create` (`collections/mod.rs:798-844`). There is no second "resident" layout. | READY NOW | same as `sekejap_open`. Not a fake of paged mode: sekejap's only mode is paged. |
...
| # | C signature | what 0.16 does | sekejap call(s) | status | ownership / error |
|---|---|---|---|---|---|
...
| 14 | `int32_t sekejap_put(db, slug, payload_json)` `:171` | `CoreDB::put` (`src/lib.rs:5316-5328`, `lib.rs:550-560`) | Split slug; `collection(name)` must exist (sekejap has no implicit collection); `Database::put(c, key, &Value)` (`collections/mod.rs:1580-1586`); FFI auto-commit. | READY NOW | `0` / `-1`. Payload must be a JSON object (`collections/mod.rs:1570-1571`). |
| 15 | `long sekejap_put_many(db, rows_json)` `:179` | JSON object `{slug: payload}` → `put_value_bulk` (`src/lib.rs:5453-5457`, `lib.rs:569-583`) | Parse object; N × `put` then one `commit` (OPS §7: sekejap is already "many puts, then one commit" — `OPS_CONTRACT.md:369-373`). Named `begin_bulk`/`end_bulk` scope: READY AFTER D1 (DML) (`OPS_CONTRACT.md:375-376`, brief-dml C). | READY NOW as N puts + one commit. The D1 bulk **scope** is DML. | count or `-1`. Failed batch: sekejap `rollback` (`collections/mod.rs:1825-1826`); stronger than 0.16 leaving the prefix stored (`OPS_CONTRACT.md:385-390`). |
...
| 19 | `int32_t sekejap_unlink(db, from, to, edge_type)` `:210` | `CoreDB::unlink` (`src/lib.rs:5849`, `lib.rs:647-660`) | Resolve slugs; interned type id; `Database::delete_edge(EdgeKey {…})` (`index/graph/mod.rs:1510-1522`) | READY NOW | `0` / `-1`. Miss of the edge is `Ok(false)` in sekejap (`:1518-1522`); C returns `0`. |
...
| 24 | `char *sekejap_schema_ddl(db, collection)` `:242` | `CoreDB::schema_ddl` from declared schema (`src/lib.rs:8262-8274`, `lib.rs:714-722`); `NULL` if no schema | `collection_info` (`collections/mod.rs:1359-1369`) → render `CREATE TABLE` from `layout.fields` + `declared`. `NULL` if the name is unknown. SQL `SHOW CREATE TABLE`: LATER (`QL_CONTRACT.md:62`). | READY NOW as a renderer over `CollectionInfo` | heap string or `NULL` (clean miss, `last_error` cleared — same as 0.16's `Ok(d.inner.schema_ddl(c))` `lib.rs:720`) |

### 1.5 Maintenance on `SekejapDb`

| # | C signature | what 0.16 does | sekejap call(s) | status | ownership / error |
|---|---|---|---|---|---|
| 25 | `int32_t sekejap_compact(db)` `:98` | `CoreDB::compact` rewrite snapshot, truncate WAL (`src/lib.rs:5980-5982`, `lib.rs:321-340`) | `Database::checkpoint` (`collections/mod.rs:1813-1819`; `QL_CONTRACT.md:55`). `Ok(false)` while a reader holds a slot: C should return `0` and not wait (deferred is success of the call, matching "never waits"). SQL `COMPACT`: LATER (`QL_CONTRACT.md:55`). | READY NOW as `checkpoint` | `0` / `-1`. Deviation vs 0.16: does not rewrite payloads; folds committed WAL (`collections/mod.rs:1810-1812`). |
| 26 | `void sekejap_trim_memory(db)` `:104` | `CoreDB::trim_memory` shrink maps, return heap (`src/lib.rs:6701-6705`, `lib.rs:347-352`) | `Database::memory_report` / `trim_memory` not built (`OPS_CONTRACT.md:323-355`, `E3_PARITY.md:161`). | LATER (`OPS_CONTRACT.md:36` §6.3; `E3_PARITY.md:161` row 87) | void. Until built: no-op is a fake — **refuse by leaving the symbol and doing nothing only if the header says it cannot fail**. The 0.16 symbol cannot fail (`void`). Keep the no-op **and** document it as "caches already bounded; nothing to trim" (OPS §6.3: sekejap holds nothing proportional to rows — `OPS_CONTRACT.md:347-353`). That is not a fake of reclaim; it is the honest empty report. |
| 27 | `int32_t sekejap_sync(db)` `:110` | `CoreDB::sync` fsync WAL (`src/lib.rs:7358-7362`, `lib.rs:359-361`) | `Database::commit` is the FULL barrier (`collections/mod.rs:1796-1808`). If auto-commit already ran, a second `commit` on a clean handle is the sekejap "nothing pending" path. | READY NOW as `commit` | `0` / `-1` |
...
0.16: `SekejapEngine` wraps `sekejap::engine::Engine` (`lib.rs:770-786`,
`src/engine/mod.rs:1-13`): `Send + Sync`, parallel readers, one writer,
thread-local errors (`sekejap.h:29-31`, `lib.rs:764-768`).

sekejap: `ServiceDatabase` is OPS §1, **not built**
(`dist/src/service/mod.rs:4-7`, `OPS_CONTRACT.md:43-78`, `E3_PARITY.md:138` row
74). D1 worker SERVICE (`brief-service.md`) builds it in `dist/src/service/`
over `Mutex<Database>` + `RwLock<Arc<Database>>` snapshots.
...
| 32 | `SekejapEngine *sekejap_engine_open_memory(void)` `:268` | `Engine::memory()` over `CoreDB::new()` (`src/engine/mod.rs:297-309`, `src/lib.rs:2465-2466`, `lib.rs:877-880`). Header: "Never fails → non-null" (`sekejap.h:267`). | sekejap `Database` is path-backed (`create`/`open` `collections/mod.rs:798-844`). No in-memory store. OPS does not name one. Law 1 is disk-first (`OPS_CONTRACT.md:21-25`). Creating a temp directory would be a fake of "ephemeral memory". | REFUSED (sekejap has no in-memory engine; disk-first, no `Database::new`) | Header claims non-null. Honour the **sentinel contract** over the comment: return `NULL`, set thread-local `"sekejap_engine_open_memory refused: sekejap has no in-memory store (OPS_CONTRACT / Law 1)"`. Do not tempdir. |
...
| 38 | `long sekejap_engine_flush(e)` `:311` | drain write buffer, apply, maybe compact (`src/engine/mod.rs:597-602`, `lib.rs:968-970`); `Ok(0)` if no buffer | sekejap has no statement buffer. Map to "commit unpublished writer work + `publish`". Empty pending → `0`, matching the 0.16 empty buffer (`src/engine/mod.rs:600-602`). | READY AFTER D1 (SERVICE) | count or `-1`. Do not invent a SQL buffer. |
...
### 2.2 What sekejap should do (keep the same header)

0.16 already uses JSON text, so JSON text is the encoding
(brief: "rows as JSON text is acceptable only if the prior engine does
that"). Do **not**
replace `char *sekejap_query` with a binary cursor. The header stays.

sekejap's native answer is columnar, not a JSON bag:
...
**Envelope (same as 0.16):** a JSON array string.

**Object per row (the one documented change inside the envelope):**

| 0.16 C ABI | sekejap C ABI (same `char*`) |
|---|---|
...
|---|---|
| `Missing` | omit the key (sekejap Missing ≠ Null; omitting matches "field not in this row") |
| `Null` | `null` |
| `Bool` | `true`/`false` |
| `Int` | number |
| `Float` | number |
...
1. **`SekejapDb*`:** one thread at a time, or the caller serialises. The FFI
   will not put a mutex on it (0.16 did not). Handle-local `last_error` would
...
**SUPERSEDED BY WHAT SHIPPED.** The proposal below put the C ABI inside
`sekejap-dist` as `dist/src/ffi/`, over `Database` and `SqlDatabase`
directly. What shipped is a SEPARATE crate at `dist/ffi` -- `sekejap-capi`,
lib name `sekejap`, `crate-type = ["cdylib", "staticlib"]` -- built over the
PUBLISHED `sekejap` crate rather than over the layers, because a `cdylib` and
an `rlib` are different artifacts with different consumers and the published
crate must not carry a `cbindgen` build dependency (`docs/LAYERS.md`). The
crate name, the lib name and the `cbindgen.toml` shape below are what
shipped; the module list and the `[dependencies]` block are not. The files
that exist are `dist/ffi/{Cargo.toml,build.rs,cbindgen.toml,Makefile,sekejap.pc.in}`,
`dist/ffi/src/`, `dist/ffi/include/sekejap.h`, `dist/ffi/examples/smoke.c` and
`dist/ffi/tests/abi.rs`.

The paragraph this section opened with is also out of date, and is kept only
so the proposal reads as it was written: `dist/src/service/` and
`dist/src/pg/` are no longer empty, and the wrappers are no longer merely
listed -- `dist/bindings/README.md` says per target what is ported and what
is published.
...
The 0.16 crate is already named `sekejap-capi` with `lib.name = "sekejap"`
(`wrappers/c/Cargo.toml:2-13`). Keep both names.

`mod.rs` sketch — a module list, not a program:
...
Taken from 0.16's `wrappers/c/cbindgen.toml:1-43`, with the engine ifdef
removed because the committed 0.16 header already emits `engine_*`
unconditionally
...
Do **not** keep 0.16's `[defines] "feature = engine" = "SEKEJAP_ENGINE"`
(`wrappers/c/cbindgen.toml:36-39`): wrapping `engine_*` in `#ifdef` would
break Go/Swift/Kotlin that include the header as-is.

`build.rs` can be copied from 0.16's `wrappers/c/build.rs:1-34` (best-effort
cbindgen; committed header is the fallback).

### 4.3 Wrapper-by-wrapper porting

**SUPERSEDED BY §8.1.** Every "unchanged" in the table below was true while
the header was 0.16's. It is not true of 0.17: the slug argument is gone, the
engine handle is gone, and three symbols became refusals, so every wrapper
has glue to re-point. What each one actually did is `dist/bindings/README.md`.
The table is kept because its "binds via" and "symbols used" columns are the
record of what each wrapper depended on.

0.16 consumption (`wrappers/README.md:3-17`): most native bindings sit on
`libsekejap`; Python and Node use language-native FFI.

| wrapper | binds via | against sekejap `libsekejap` with the **same header** | glue to re-point |
|---|---|---|---|
...
| method | line | in `sekejap.h`? | sekejap |
|---|---|---|---|
...
## 5. What sekejap must not reproduce, and names to keep
...
| item | reason |
|---|---|
| A binary / columnar cursor replacing `char *sekejap_query` | 0.16 is JSON (`lib.rs:150-159`); changing the header breaks Go/Swift/Kotlin/C/Lua/C# |
| `#ifdef SEKEJAP_ENGINE` around `engine_*` | the committed 0.16 header emits them unconditionally (`sekejap.h:261-328`) |
...
| The 0.16 payload-bag `{"_slug":...}` as the only row shape | sekejap rows are projected columns (`lang/src/lib.rs:272-276`); keep the JSON **array** envelope, not the bag |
...
| Per-write fsync that the 0.16 WAL-appends implied, implemented as a weakened sekejap commit | OPS §7: same FULL barrier; weakened bulk durability is T3 (`OPS_CONTRACT.md:380-383`) |
| `DECLARE … BINARY CURSOR` symbols in this header | T2 p3-wire (`QL_CONTRACT.md:59`); a later header |

### 5.3 Header comments sekejap should edit (same symbols)

The comment on `sekejap_query` that each element is "the row's payload object
(or `{"_slug": "..."}`)" (`sekejap.h:73-75`) is 0.16-specific. In sekejap, the
...
(`sekejap.h:267`) is false on sekejap. Keep the symbol; document the `NULL` +
...
**HISTORICAL.** Every "READY AFTER D1" row below has landed: the DML, SERVICE
and SCHEMA workers finished, and `docs/dist/C_ABI.md` is the surface that
resulted. The "LATER" and "REFUSED" rows are the ones to read against §8,
which says what each symbol became.
...
NOT a Rust port of this header: the C ABI keeps 0.16's shape so existing C
callers stay valid, and the Rust crate is sekejap's own design -- collection
and key instead of a slug string, `serde_json::Value` in and out, `$n`
parameters, one `Result`, and a refusal by name wherever sekejap has no atomic.
...
|---|---|---|
| 1, 2 open / open_paged | yes | `Db::open`, `Db::open_with` (sekejap has one store; there is no second "resident" layout) |
...
| 21 node_count | yes | `Db::count_rows(collection)` reads the live row-count record (`core/engine/src/collections/row_count.rs`) and walks only where a database has none; `Db::scan_count_rows(collection)` and `Db::scan_count_all_rows()` are the explicit walks, NAMED as scans |
...
| 26 trim_memory | REFUSED, by absence and by documentation | `RUST_API.md` §7 lists it: sekejap holds nothing proportional to rows to trim (OPS §6.3) |
...
| 32 engine_open_memory | REFUSED | sekejap is disk-first; `Db::open` takes a directory (`RUST_API.md` §1, §7) |
...
a row the Rust facade answers differently, not a row sekejap cannot serve.

---

## 8. The 0.17 header column

`dist/ffi` (`libsekejap` 0.17.0, `docs/dist/C_ABI.md`) is the C ABI that
SHIPPED, built over the published crate's `Db` rather than over `CoreDB`. It
keeps §0's ownership rules -- opaque handles, borrowed strings in, owned
strings out freed by `sekejap_string_free`, `NULL`/`-1` sentinels, no panic
across the boundary, the static `sekejap_version` -- and §2's JSON envelope.
What it does not keep is the 0.16 FUNCTION LIST: a row is addressed by
collection and key as two arguments rather than by one slug string, service
mode is the same `SekejapDb*` opened another way rather than a second handle
type, and `sekejap_last_error` gained `sekejap_last_error_code`, the closed
enumeration §0 said a wrapper needed and did not have.

The column below says, for each row of §1, what became of that symbol. 59
functions in the 0.17 header; 41 in the 0.16 one.

| §1 row | 0.17 header |
|---|---|
| 1 `sekejap_open` | `sekejap_open(const char *path)` -- unchanged in name and signature |
| 2 `sekejap_open_paged` | DROPPED. sekejap has one store, the page WAL, and it is already paged: a second opener would be two names for one thing. `sekejap_open_with_config(path, config_json)` is the opener that takes `budget_bytes`, `io` and `sync` |
| 3 `sekejap_close` | `sekejap_close(SekejapDb *db)` -- unchanged |
| 4 `sekejap_open_read_only` | DROPPED. A read-only DATABASE has no separate opener here: `sekejap_open_service` gives every reader a published snapshot, which is the same guarantee under the mode that also carries the change feed |
| 5, 7 `sekejap_execute`, `_execute_params` | MERGED into `sekejap_execute(db, sql, params_json)`. One call takes the parameters whether or not there are any; a `NULL` params pointer is no parameters |
| 6, 8 `sekejap_query`, `_query_params` | MERGED into `sekejap_query(db, sql, params_json)`. Same JSON-array envelope; the object per row is keyed by COLUMN NAME, which is §2.2's one documented change inside the envelope |
| 9 `sekejap_prepare` | `sekejap_prepare(db, sql)` -- parses here, compiles on the first bind |
| 10 `sekejap_query_prepared` | `sekejap_stmt_query(stmt, params_json)`. The `db` argument is gone: the statement handle already borrows its database. `sekejap_stmt_execute` is the writing half, and `sekejap_stmt_rebindable` says whether a further bind compiles nothing -- a fact 0.16 had no way to ask |
| 11 `sekejap_stmt_free` | `sekejap_stmt_free(SekejapStmt *stmt)` -- unchanged |
| 12 `sekejap_show` | KEPT AS A REFUSAL. The symbol is there and always answers `NULL` with `SekejapStatus_Refused`, naming `QL_CONTRACT.md` §2 and pointing at `sekejap_collections` and `sekejap_describe`, which answer the same questions as data |
| 13 `sekejap_get` | `sekejap_get(db, collection, key)`. The slug split by `/` is gone: two arguments cannot be swapped silently. A miss is still `NULL`, and `sekejap_last_error_code` now tells a miss from a failure without parsing text |
| 14 `sekejap_put` | `sekejap_put(db, collection, key, document_json)` |
| 15 `sekejap_put_many` | `sekejap_put_many(db, collection, rows_json)`. The `{slug: payload}` object became a JSON array of `{"key", "doc"}` into ONE collection, which is what "N puts, one commit" addresses |
| 16 `sekejap_remove` | RENAMED `sekejap_delete(db, collection, key)`, and it now ANSWERS whether the row was there (`1`/`0`) instead of reporting success either way |
| 17, 18 `sekejap_link`, `_link_meta` | `sekejap_link(db, from_collection, from_key, edge_type, to_collection, to_key)` and `sekejap_link_with(..., properties_json)`. Both endpoints must EXIST: a missing one is `SekejapStatus_UnknownRow`, not a dangling hash |
| 19 `sekejap_unlink` | `sekejap_unlink(db, ...)`, answering `1`/`0` for whether the edge was there |
| 20 `sekejap_contains` | RENAMED `sekejap_exists(db, collection, key)`, `1`/`0`/`-1` |
| 21 `sekejap_node_count` | `sekejap_count_rows(db, collection)`, which reads the LIVE row-count record and walks only where a database has none, and `sekejap_scan_count_rows(db, collection)`, the explicit walk NAMED as one. There is no all-collections node count in the header: a count is of a collection |
| 22 `sekejap_edge_count` | `sekejap_scan_count_edges(db)` -- the same walk under a name that says it is one, because sekejap keeps no O(1) edge counter (`OPS_CONTRACT.md` §6.1) |
| 23 `sekejap_collection_names` | RENAMED `sekejap_collections(db)`, over `Database::list_collections` |
| 24 `sekejap_schema_ddl` | REPLACED BY DATA: `sekejap_describe(db, collection)` answers the fields, kinds, declared spellings, indexes and live row count as a JSON object. Rendering `CREATE TABLE` text from them is the caller's, because a renderer that only round-trips some declarations is the fake |
| 25 `sekejap_compact` | SPLIT. `sekejap_checkpoint(db)` is the real operation -- fold the committed WAL, `1`/`0` for folded or DEFERRED. `sekejap_compact` is KEPT AS A REFUSAL, because sekejap has no payload-rewriting compaction and the 0.16 name promised one |
| 26 `sekejap_trim_memory` | KEPT AS A REFUSAL, and its type changed from `void` to `int32_t` so it CAN refuse. 0.16 could only no-op, which §1 #26 called the honest empty report; a symbol that cannot fail cannot say why it did nothing |
| 27 `sekejap_sync` | DROPPED. Every plain write commits with a FULL barrier before it returns, so there is nothing left to flush; `sekejap_publish(db)` is the separate step that makes a commit visible to a snapshot reader, and `sekejap_tx_commit` is the barrier when many writes share one |
| 28 `sekejap_last_error` | `sekejap_last_error(const SekejapDb *db)`, same signature, **now THREAD-LOCAL** rather than handle-local. The handle is accepted and ignored. That is the one behaviour change in §0's table, and it is what lets a failed `sekejap_open` -- which has no handle -- report at all |
| 29 `sekejap_string_free` | `sekejap_string_free(char *s)` -- unchanged, and still never for `sekejap_version` |
| 30 `sekejap_version` | `sekejap_version(void)` -- unchanged, static. `sekejap_format_version(void)` is new beside it: the disk format, 2 |
| 31-40 `sekejap_engine_*` | DROPPED AS A SECOND HANDLE, KEPT AS A MODE. `sekejap::Db` is `Send + Sync`, so one `SekejapDb*` may be shared across threads and there is nothing for a second handle type to add. `sekejap_open_service(path)` is `ServiceDatabase` (OPS §1-§5) behind the same calls; `sekejap_statement_timeout_ms`, `sekejap_cancel`, `sekejap_clear_interrupt`, `sekejap_subscribe`, `sekejap_next_change` and `sekejap_unsubscribe` are the service's own surface, REFUSED BY NAME on a single-mode handle. `engine_flush` has no successor: a commit is the barrier and there is no statement buffer to drain |
| 32 `sekejap_engine_open_memory` | KEPT AS A REFUSAL, under the name `sekejap_open_memory(void)`. Always `NULL` with `SekejapStatus_Refused`; no temporary directory, which §5.2 named as the fake |
| 41 `sekejap_engine_last_error` | DROPPED. `sekejap_last_error` is thread-local now, so the two error slots §3.3 said not to mix are one slot |

New in 0.17, with no 0.16 row: `sekejap_open_with_config`,
`sekejap_last_error_code`, `sekejap_scan_open`/`_next`/`_close` (the lazy
collection walk), `sekejap_query_open`/`_next`/`_close` (a paged answer),
`sekejap_explain`, `sekejap_stmt_execute`, `sekejap_stmt_rebindable`,
`sekejap_neighbours`, `sekejap_create_collection`, `sekejap_drop_collection`,
`sekejap_describe`, `sekejap_tx_begin`/`_put`/`_delete`/`_link`/`_execute`/
`_commit`/`_rollback`, `sekejap_publish`, `sekejap_storage`,
`sekejap_format_version`, and the six service calls.

### 8.1 What every wrapper must re-point

§4.3 said the C, C++, Go, Swift, Kotlin, Lua, C# and React Native wrappers
were "unchanged" because the header was. That held while the header was
0.16's. It is not true of 0.17: the slug argument is gone, the engine handle is
gone, and three symbols became refusals. Every wrapper in `dist/bindings`
therefore has glue to re-point, and `dist/bindings/README.md` carries the
list per target. None of them is built from this workspace.
