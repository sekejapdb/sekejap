...
Drafted 2026-09-20. Companion to `docs/QL_CONTRACT.md` (what the language is)
and `docs/GRAPH_CONTRACT.md` (what the graph is). This document fixes the
...
The reason it exists: `docs/E3_PARITY.md` counts 33 e3 capabilities e4's
...
Tiers are `docs/QL_CONTRACT.md`'s: **T1** compiles to an atomic that exists at
...
Laws are `CONTRACT.md` / `docs/FOUNDATION_TEST_STANDARD.md`: L1 disk-first
...
blocks the writer's checkpoint. `docs/ARCHITECTURE.md` §1.3 already states the
...
in its own `src/service.rs`; `docs/SOURCE_LAYOUT.md` gains the row.
...
`docs/ARCHITECTURE.md` §2), so the service must refuse to mint a snapshot past
...
through the committed-WAL overlay (`docs/ARCHITECTURE.md` §1.3,
...
the `SET LOCAL` path that already exists (`src/sql/compile.rs:814`).
...
`docs/QL_CONTRACT.md` makes for a budget refusal.
...
the `SHOW STATUS` sugar over it (`docs/QL_CONTRACT.md` §2).

**Stated cost.** Everything above is O(1). The **node and edge counts are
not**: a count without a filter is a scan, which `docs/QL_CONTRACT.md` §6
...
(`docs/ARCHITECTURE.md` §1.2), so one ordered walk attributes every byte, and
...
falsifiable rather than asserted. `docs/ARCHITECTURE.md` §6 names the
...
(`src/sql/compile.rs:814`, where `ef_search` and
...
| L2 Work ∝ change | A bulk batch is one barrier for N rows, not N barriers; the predicate-driven `UPDATE`/`DELETE` of `docs/QL_CONTRACT.md` §2 walks matches, not collections. | §7 |
