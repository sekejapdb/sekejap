...
Drafted 2026-09-20. Companion to docs/core/GRAPH_CONTRACT.md (engine semantics)
and docs/dist/OPS_CONTRACT.md (the runtime and ops surface: service mode, publish,
...
| spatial | PostGIS 3.6 function names and unit semantics | docs/core/SPATIAL_FUNCTIONS.md, tests/spatial_postgis_conformance.rs |
...
| `BEGIN BULK`, `END BULK` | T1 | `Database::begin_bulk` / `end_bulk` (`core/engine/src/collections/write_set.rs`), `docs/dist/OPS_CONTRACT.md` §7. **The spelling is e4's and is fixed here:** `BEGIN BULK` / `END BULK`, not `BEGIN`/`COMMIT`, because the scope is not a transaction -- the single writer is already inside one -- it is a deferral of the DURABILITY POINT to the matching close. Scopes nest and the counter says how deep; only the OUTERMOST close calls `commit`, with the same FULL barrier and the same publication as any other commit. A close with no scope open is REFUSED rather than absorbed (e3 absorbed it because the call arrives from FFI; this is not that boundary), and a `ROLLBACK` discards the batch and the scope together, which is e4's all-or-nothing batch. |
...
| `SHOW STATUS`, `SHOW STORAGE` | T2 | `docs/dist/OPS_CONTRACT.md` §6: the runtime facts and the per-keyspace byte report. `SHOW STORAGE` is a scan by definition and says so. |
...
| `ST_Intersects`, `ST_Within`, `ST_Contains` | T1 | Geometry filters (units per docs/core/SPATIAL_FUNCTIONS.md) |
...
2. Aggregates (§4.7) — DONE, `src/query/aggregate.rs`; date/time and string
   functions (§4.1, §4.2) — DONE, `lang/src/functions.rs` plus the expression
...
10. `docs/dist/OPS_CONTRACT.md` in its own order: service mode and publish, then
