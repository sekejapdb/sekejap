...
"killer feature" for high-traffic *server* use (app): letting many reads run
...
That's the "app feels frozen" the app note describes. Snapshot reads remove
...
- **Cheap in paged mode; expensive in resident mode.** The whole "free base"
  ...
  snapshot would mean copying it. **app currently uses resident mode**, so
  adopting this feature pairs naturally with moving app to paged mode (already
