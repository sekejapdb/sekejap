...
| `src/query/membership.rs` | `MembershipSet`, `MembershipBudget`, `build_scalar_range_set`, `build_point_set`, the posting probes and `ensure_membership_sets`; `NodeGate`, the per-hop node predicates of a traversal, and `StandaloneNodeGate` for the traversal atomic. | `docs/QL_CONTRACT.md` §6 "Execution guarantees"; `docs/GRAPH_CONTRACT.md` §4.3 |
