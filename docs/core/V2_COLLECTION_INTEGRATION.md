...
`docs/core/V2_FOUNDATION_LOOP.md`. This is integration toward release, not a
...
Kernel edits (authorized, smallest possible): `core/kernel/src/io.rs` gained
...
`io.rs`); `core/kernel/src/limits.rs` made `ResourceLimits::{encode, decode}`
...
- `src/pagewal.rs`, `src/pagewal_hint_tests.rs` (new), `src/pagewal/repair.rs`
  ...
  `src/collections.rs`, `dist/src/cli/collection_inspect.rs` (now a V2 diagnostic:
  reads through the overlay, reports WAL/hint bytes), `docs/core/COLLECTIONS.md`,
  `tests/collections.rs` (original 64 KiB policy restored),
  `tests/collection_pagewal.rs`, `core/kernel/src/io.rs`, `core/kernel/src/limits.rs`.

## Linux validation (run against candidate r3; results in docs/core/V2_FOUNDATION_LOOP.md)
...
`docs/core/V2_FOUNDATION_LOOP.md` for the actual pass counts, hashes and numbers.
