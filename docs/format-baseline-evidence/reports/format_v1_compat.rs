//! Lean format-v1 compatibility suite. Fixtures in `docs/format-v1-fixtures/`
//! and `docs/format-v1-baseline/` are mandatory preserved reference files;
//! this file never regenerates either corpus.
//!
...
use e4_prototype::collections::{CollectionId, Database, EntityId};
use kernel::{
    io::IoMode,
...
const FIXTURES: &str = "docs/format-v1-fixtures";
// Locks the preserved corpus, including its independent expected-result manifests.
const INDEX_SHA256: &str = "e274ef52d6de6e5073bd6f37b753a52db9bf6ead0affacc3db571b2dbe4da64e";
const BASELINE_FIXTURES: &str = "docs/format-v1-baseline";
// Captured by the clean committed 59d1cbc reference generator on Linux.
// Preserve this independently of the original codec-only prototype corpus.
const BASELINE_INDEX_SHA256: &str =
    "6bd933a1a63c6f2c2c3af0ac6f6e4d62c29c011a5ccdcbf66d32835fdb88ec26";
...
fn require_fixtures_root(root: PathBuf) -> PathBuf {
    assert!(
        ...
        "required preserved format-v1 fixtures missing at {}; compatibility qualification cannot skip fixtures",
        root.display()
    );
    root
}
...
#[test]
#[should_panic(expected = "required preserved format-v1 fixtures missing")]
...
fn copy_fixture(fx: &Fixture, label: &str) -> tempfile::TempDir {
    ...
    let dst = tempfile::Builder::new()
        .prefix(&format!("format-v1-{}-{label}-", fx.name))
        .tempdir_in(base)
        .unwrap();
