...
`src/bin/v2_foundation_bench.rs` is a fair, streaming, mixed-data comparison
...
`version: u64`** (same convention as `person()` in `src/bin/people.rs` and
`doc()` in `src/bin/collections.rs`): nothing about a row needs to be held
...
`src/bin/collections.rs`'s two-collection harness, not here; this is a
...
`docs/FOUNDATION_TEST_STANDARD.md` (`W-UPDATE` 20% per round, `W-DELETE` 10%
per round, `W-MIXED` update 20%/delete 10%/insert 10% per round) and in
`src/bin/collections.rs`'s `doc()` function (disjoint `slot % 5 == 0`
...
`src/bin/people.rs`). It performs no path allow-listing beyond that — the
...
- This measures one collection with one schema shape; it is not a claim
  ...
  `src/bin/collections.rs`) or about query/SELECT performance of any kind.
