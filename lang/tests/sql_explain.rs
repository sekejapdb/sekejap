...
//!
//! Three things are asserted, and they are the three `docs/lang/QL_CONTRACT.md` §6
//! promises:
//!
...
fn weighted_graph(f: &mut fixture::Fixture) -> sekejap_core::collections::EdgeTypeId {
    ...
    f.db.commit().unwrap();
    let ids: Vec<sekejap_core::collections::EntityId> = f
        .keys
        .iter()
...
fn a_boolean_filter_is_answered_before_the_row_is_read() {
    ...
    // (`docs/lang/QL_CONTRACT.md` §3), and it no longer costs the walk its shape
    // either.
