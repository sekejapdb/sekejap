// sekejap — the shared, backend-agnostic typed TS core (schema-as-code, no build
// step). Imports NO native backend: the backend is injected at `open({ native })`,
// so this exact source bundles into every package (sekejap on Node, @sekejap/
// react-native on device). Each package wraps `open` with its default backend.
 ...
 */
export function open<S extends Schema>(
  path: string,
