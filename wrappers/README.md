...
| **Dart/Flutter** | [pub.dev](https://pub.dev) | `dart pub add sekejap` · `flutter pub add sekejap` | `dart pub publish`. Published under the *verified publisher* **`app.com`** (the umbrella domain, shared by all products) |
...
| **Kotlin/Java** | [Maven Central](https://central.sonatype.com) | Gradle `implementation("com.app:sekejap:0.13.0")` | Publish via the Sonatype **Central Portal**. groupId **`com.app`** (reverse-DNS of the `app.com` umbrella; verifying the apex grants `com.app.*` for every product). Verify the domain once |
